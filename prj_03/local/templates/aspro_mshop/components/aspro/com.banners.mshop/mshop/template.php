	...
	<div class="start_promo <?=($arResult["OTHER_BANNERS_VIEW"]=="Y" ? "other" : "normal_view");?>">
		...
		<?foreach($arResult["ITEMS"] as $arItem):?>
			<?if($arItem["DETAIL_PICTURE"]["SRC"] || $arItem["PREVIEW_PICTURE"]["SRC"]):?>
				...
				if (!empty($arItem["PROPERTIES"]["BANNER_SIZE"]["VALUE_XML_ID"])) {
					$widthClass = $arItem["PROPERTIES"]["BANNER_SIZE"]["VALUE_XML_ID"];
				} else {
					$widthClass = "normal";
				}
				
				if (!empty($arItem["PROPERTIES"]["STYLE"]["VALUE"])) {
					$widthClass = '';
					$widthStyle = ' style="width:'.$arItem["PROPERTIES"]["STYLE"]["VALUE"].'%;"';
				}	else {
					$widthStyle = '';
				}	
				...
				?>
				<div class="item s_<?=$i;?> <?=($isUrl ? "hover" : "");?> <?=$widthClass?><?=$borderClass?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>"<?=$widthStyle?>>
