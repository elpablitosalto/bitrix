    ...
    <? if ($bSectionsLeft && $arAllSections): ?>
        <div class="top_block_filter_section">
            <div class="title"><a class="dark_link" title="<?= GetMessage("FILTER_ALL_SECTON"); ?>"
                                  href="<?= $APPLICATION->GetCurPage(false) . '?q=' . htmlspecialcharsbx($_REQUEST['q']) ?>"><?= GetMessage("FILTER_SECTON"); ?></a>
            </div>
            <div class="items">
                <? foreach ($arAllSections as $key => $arTmpSection): ?>
                    <div class="item <?= ($sectionIDRequest ? ($key == $sectionIDRequest ? 'current' : '') : '') ?>"><a
                                href="<?= $APPLICATION->GetCurPage(false) . '?q=' . htmlspecialcharsbx($_REQUEST['q']) . '&section_id=' . $key ?>"
                                class="dark_link"><span><?= $arSections[$key]["NAME"]; ?></span><span><?= $arTmpSection["COUNT"]; ?></span></a>
                    </div>
                <? endforeach; ?>
            </div>
        </div>
...
if (is_array($arElements) && !empty($arElements)) {
    ...
    if ($q) {
        ...
        <div class="my-3">
            <h2 class="h3">Вот, что мы нашли по запросу «<?= $q; ?>»:</h2>
        </div>
        <?php
    }
    ?>
    ...
    <div class="catalog vertical">
        ...
        <div class="sort_header view_<?= $display ?>">
            ...
            <div class="sort_filter">
                ...
                if (in_array("AVAILABLE", $arSorts)) {
                    $arAvailableSort["AVAILABLE"] = array("PROPERTY_SORT_AVAILABLE", "desc");
                }
                if (in_array("NAME", $arSorts)) {
                    $arAvailableSort["NAME"] = array("NAME", "asc");
                }
                if (in_array("HIT", $arSorts)) {
                    $arAvailableSort["HIT"] = array("PROPERTY_SORT_HIT", "desc");
                }
                if (in_array("NEW", $arSorts)) {
                    $arAvailableSort["NEW"] = array("PROPERTY_SORT_NEW", "desc");
                }
                if (in_array("RECOMMEND", $arSorts)) {
                    $arAvailableSort["RECOMMEND"] = array("PROPERTY_SORT_RECOMMEND", "asc");
                }
                ...
                foreach ($arAvailableSort as $key => $val):?>
                    ...
                    <a rel="nofollow"
                       ...
                       class="sort_btn <?= ($sort == $key ? 'current' : '') ?> <?= $sort_order ?> <?= $key ?>"
                       rel="nofollow">
                        <i class="icon"
                           title="<?= GetMessage('SECT_SORT_' . $key) ?>"></i><span><?= GetMessage('SECT_SORT_' . $key) ?></span><i
                                class="arr"></i>
                    </a>
                <? endforeach; ?>
                <?
                if (in_array($sort, ["PRICE", "AVAILABLE", "HIT", "NEW", "RECOMMEND"])) {
            ...
            <div class="sort_display">
                ...
                foreach ($arDisplays as $displayType):?>
                    <a rel="nofollow"
                       href="<?= $APPLICATION->GetCurPageParam('display=' . $displayType, array('display')) ?>"
    ...
    ?>
    <h3 class="h5">Если по Вашему запросу ничего не нашлось, то обязательно позвоните нашим менеджерам <a
                href="tel:+78633332256">+7 863 333-22-56</a></h3>
    <?php
