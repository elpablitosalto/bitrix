...
function recalcBasketAjax(params) {
	//alert('2');
	console.log(basketPoolQuantity);
	//basketPoolQuantity.setProcessing(false);
	//alert(basketPoolQuantity.isProcessing());
...
// Мой код -->
var ajaxTimerId_updateBasketCustom = 0;
function updateBasketCustom() {
	if (Number(ajaxTimerId_updateBasketCustom) > 0) {
		clearTimeout(ajaxTimerId_updateBasketCustom);
	}
	if (true) {
		ajaxTimerId_updateBasketCustom = setTimeout(function () {
			location.reload();
		}, 1500);
	} else {
		ajaxTimerId_updateBasketCustom = setTimeout(function () {
			updateBasketCustom_ajax();
		}, 1500);
	}
}
function updateBasketCustom_ajax() {
	//alert('2');
	$.ajax({
		...
		success: function (data) {
			//console.log(data);
			//$('.js_basket_wrapper').html($(data).find('.js_basket_wrapper').html());
			$('#content').html($('<div>' + data + '</div>').find('#content').html());
			initBasket();
			return false;
		}
	});
