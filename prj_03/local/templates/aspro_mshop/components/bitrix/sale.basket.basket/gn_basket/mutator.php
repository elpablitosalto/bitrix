...
foreach ($this->basketItems as $row)
{
	$rowData = array(
		...
		// Мой код -->
		'SHOW_DEL_ICON' => $row['PRODUCT_ID'] != SWP_PRODUCT_ID,
		'SHOW_UPDATE_INPUT' => $row['PRODUCT_ID'] != SWP_PRODUCT_ID,
		'SHOW_ONLY_QOUNT' => $row['PRODUCT_ID'] == SWP_PRODUCT_ID,
		// <-- Мой код
	);

	if(!empty($rowData["PROPS"])){
	    foreach ($rowData["PROPS"] as $arProperty) {
	        if($arProperty["CODE"] == "MIN_PACK_VALUE_PRICE" && !empty($arProperty["VALUE"])) {
                $propValue = preg_replace('/[^\d]+/i', '', $arProperty["VALUE"]);
                if(!empty($propValue)) {
                    $rowData['SUM_PRICE'] = PriceMaths::roundPrecision($rowData['SUM_PRICE'] + intval($propValue));
                    $rowData['SUM_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($rowData['SUM_PRICE'], $rowData['CURRENCY'], true);
                    //
                    $result['allSum'] = PriceMaths::roundPrecision($result['allSum'] + intval($propValue));
                    $result['allSum_FORMATED'] = CCurrencyLang::CurrencyFormat($result['allSum'], $rowData['CURRENCY'], true);
                }
            }
        }
    }
