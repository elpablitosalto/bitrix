...
?>
	...
	<? if ($iSectionsCount > 0): ?>
		...
		<? $myWishSubcategries = 'Y';

		//Очистить фасетный индекс
		//Bitrix\Iblock\PropertyIndex\Manager::DeleteIndex($arParams["IBLOCK_ID"]);
		//Bitrix\Iblock\PropertyIndex\Manager::markAsInvalid($arParams["IBLOCK_ID"]);

		if ($myWishSubcategries == 'Y') {
	...
	<? if ($arParams['INCLUDE_SUBSECTIONS'] !== "N" || !$iSectionsCount): ?>
		<? if ($isAjaxFilter != "Y"): ?>
			...
			<div class="inner_wrapper">
				<? if ($arSeoItem): ?>
					<div class="seo_block">
						<? if ($arSeoItem["DETAIL_PICTURE"]): ?>
							<img class="js_lazy img-responsive" data-src="<?= CFile::GetPath($arSeoItem["DETAIL_PICTURE"]); ?>" alt="<?= $arSeoItem["NAME"] ?>" title="<?= $arSeoItem["NAME"] ?>" />
						<? endif; ?>
				...
				<? } ?>
				<? if ($isAjax == "Y") {
					$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH . '/js_custom/jquery.plugin.min.js', true);
					$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH . '/js_custom/jquery.countdown.min.js', true);
					$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH . '/js_custom/jquery.countdown-ru.js', true);
					//die();
				} ?>
				<? if ($isAjax != "Y") { ?>
					<? $frame->end(); ?>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$(window).resize(function() {
					...
					$('.tab_slider_wrapp.<?= $class_block; ?> .tabs_content > li.cur').equalize({
						children: '.item-title'
					});
					$('.tab_slider_wrapp.<?= $class_block; ?> .tabs_content > li.cur').equalize({
						children: '.item_info'
...
if (
	!empty($section) &&
	(
		preg_match('/\/filter\/.*\/apply\//i', $request->getRequestUri()) ||
		!empty($request->get("display")) ||
		(!empty($request->get("sort")) && !empty($request->get("order")))
	)
) {
	\Bitrix\Main\Page\Asset::getInstance()->addString('<link rel="canonical" href="' . $section["SECTION_PAGE_URL"] . '">', true);
}
//
?>