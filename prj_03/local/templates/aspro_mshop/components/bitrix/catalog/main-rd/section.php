...
if (!empty($GLOBALS['CATALOG_USE_FILTER'])) {
	$arParams['USE_FILTER'] = $GLOBALS['CATALOG_USE_FILTER'];
}
// <-- Использовать или нет фильтр
			...
			<div class="left_block catalog <?= strtolower($TEMPLATE_OPTIONS["TYPE_VIEW_FILTER"]["CURRENT_VALUE"]) ?>">
				...
				<? if ($arParams["SHOW_SECTION_SIBLINGS"] == "Y"): ?>
					<? $APPLICATION->IncludeComponent(
						...
						"internal_sections_list-rd",
						array(
							"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
							"IBLOCK_ID" => $arParams["IBLOCK_ID"],
							"DISPLAY_PANEL" => $arParams["DISPLAY_PANEL"],
							"CACHE_TYPE" => "N", // ломает класс cur
			...
			<div class="inner_wrapper">
				...
				<div class="sort_header view_<?= $display ?>">
					...
					<div class="sort_filter">
						...
						if ($sort == "SHOWS") {
							$sort = "PROPERTY_CATALOG_SORT_RANKING";
						}
						//
						?>
					</div>
...
<? } ?>
<style>
	.prices .price_group .price_name {
		display: none;
	}

	.prices .price_group .price span.price_value:before {
		content: "от ";
	}
</style>
<?
