...
// сортировка
if(!empty($arParams["RECOMMENDATIONS_ELEMENT"])){
	$currentPropduct = $arParams["RECOMMENDATIONS_ELEMENT"];
	usort($arResult["ITEMS"], function($val_a, $val_b) use ($currentPropduct){
		if($currentPropduct["PROPERTY_MATERIAL_ENUM_ID"] == $val_a["PROPERTIES"]["MATERIAL"]["VALUE_ENUM_ID"] && $currentPropduct["PROPERTY_MATERIAL_ENUM_ID"] != $val_b["PROPERTIES"]["MATERIAL"]["VALUE_ENUM_ID"]){
			return -1;
		}elseif($currentPropduct["PROPERTY_MATERIAL_ENUM_ID"] != $val_a["PROPERTIES"]["MATERIAL"]["VALUE_ENUM_ID"] && $currentPropduct["PROPERTY_MATERIAL_ENUM_ID"] == $val_b["PROPERTIES"]["MATERIAL"]["VALUE_ENUM_ID"]){
			return 1;
		}else{
			if($currentPropduct["PROPERTY_SEZON_ENUM_ID"] == $val_a["PROPERTIES"]["SEZON"]["VALUE_ENUM_ID"] && $currentPropduct["PROPERTY_SEZON_ENUM_ID"] != $val_b["PROPERTIES"]["SEZON"]["VALUE_ENUM_ID"]){
				return -1;
			}elseif($currentPropduct["PROPERTY_SEZON_ENUM_ID"] != $val_a["PROPERTIES"]["SEZON"]["VALUE_ENUM_ID"] && $currentPropduct["PROPERTY_SEZON_ENUM_ID"] == $val_b["PROPERTIES"]["SEZON"]["VALUE_ENUM_ID"]){
				return 1;
			}else{
                if($currentPropduct["PROPERTY_TSVET_OSNOVNOY_ENUM_ID"] == $val_a["PROPERTIES"]["TSVET_OSNOVNOY"]["VALUE_ENUM_ID"] && $currentPropduct["PROPERTY_TSVET_OSNOVNOY_ENUM_ID"] != $val_b["PROPERTIES"]["TSVET_OSNOVNOY"]["VALUE_ENUM_ID"]){
                    return -1;
                }elseif($currentPropduct["PROPERTY_TSVET_OSNOVNOY_ENUM_ID"] != $val_a["PROPERTIES"]["TSVET_OSNOVNOY"]["VALUE_ENUM_ID"] && $currentPropduct["PROPERTY_TSVET_OSNOVNOY_ENUM_ID"] == $val_b["PROPERTIES"]["TSVET_OSNOVNOY"]["VALUE_ENUM_ID"]){
                    return 1;
                }else{
                    if($currentPropduct["PROPERTY_TSVET_OTDELKI_ENUM_ID"] == $val_a["PROPERTIES"]["TSVET_OTDELKI"]["VALUE_ENUM_ID"] && $currentPropduct["PROPERTY_TSVET_OTDELKI_ENUM_ID"] != $val_b["PROPERTIES"]["TSVET_OTDELKI"]["VALUE_ENUM_ID"]){
                        return -1;
                    }elseif($currentPropduct["PROPERTY_TSVET_OTDELKI_ENUM_ID"] != $val_a["PROPERTIES"]["TSVET_OTDELKI"]["VALUE_ENUM_ID"] && $currentPropduct["PROPERTY_TSVET_OTDELKI_ENUM_ID"] == $val_b["PROPERTIES"]["TSVET_OTDELKI"]["VALUE_ENUM_ID"]){
                        return 1;
                    }else{
                        return 0;
                    }
...
$arConvertParams = array();
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
        ...
        $arItem["LINKED_PRODUCTS"] = productColors::getLinkedProducts($arItem);
...
$arResult['MIN_PRICES'] = array();
$arPrices = array();
$IBLOCK_ID = false;
$arSKUIds = array();
$arSKUParentsIds = array();
foreach ($arResult['ITEMS'] as $key => $arItem) {
	if (!empty($arItem['OFFERS'])) {
		foreach ($arItem['OFFERS'] as $arSKU) {
			$arSKUIds[] = $arSKU['ID'];
			$arSKUParentsIds[$arSKU['ID']] = $arItem['ID'];
...
if (intval($IBLOCK_ID) > 0 && !empty($arSKUIds)) {
	...
	foreach ($allProductPrices as $allProductPrice) {
		//vardump($allProductPrice);
		$arResult['ALL_PRICES'][] = $allProductPrice;
		$arPrices[$arSKUParentsIds[$allProductPrice["PRODUCT_ID"]]][] = $allProductPrice['PRICE'];
	}

	foreach ($arPrices as $productId => $ar_prices) {
		$min_price = min($ar_prices);
		if ($min_price > 0) {
			//$arResult['MIN_PRICES'][$productId] = BitrixTools::getPriceFormatted($min_price);
			$arResult['MIN_PRICES'][$productId] = CurrencyFormat($min_price, "RUB");
		}
	}
}

//vardump($arResult['MIN_PRICES']);
// <-- Минимальная цена товара
?>