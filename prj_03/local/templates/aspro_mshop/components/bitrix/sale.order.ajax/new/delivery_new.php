			...
			<div class="bx_block w100 vertical">
				...
				<div class="bx_element">
					...
					<label for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>">
						...
						<?
						if (is_countable($arDelivery["LOGOTIP"]) && count($arDelivery["LOGOTIP"]) > 0):
						...
						<div class="bx_description">
							...
							<strong <?=$clickHandler?>>
								...
								<!-- <?
								if (str_contains(htmlspecialcharsbx($arDelivery["NAME"]), 'за счет получателя')):
									echo "<pre>"; print_r(htmlspecialcharsbx($arDelivery["NAME"])); echo "</pre>";
								endif;
								?> -->
								...
								<?if(isset($arDelivery["PRICE"])):?>
									<?=GetMessage("SALE_DELIV_PRICE");?>: <b>
										<?
											...
											}elseif(str_contains(htmlspecialcharsbx($arDelivery["NAME"]), 'за счет получателя')){
												echo 'оплачивается при получении';
									...
									{
										$periodText = $arDelivery["PERIOD_TEXT"];

										if (str_contains(htmlspecialcharsbx($arDelivery["NAME"]), 'Деловые линии')){
											$periodText = strval($periodText + 14);

										}

										if (preg_match('/^\d+$/', $periodText)) {
											$periodText = 'до ' . $periodText . ' дней';
										} else if (!preg_match('/от/', $periodText)) {
											$periodText = 'до ' . preg_replace('/дн. (\d+)/', '$1 дней', $periodText);
										}

										echo GetMessage('SALE_SADC_TRANSIT').": <b>". $periodText ."</b>";
										?><br /><?
									}
						...
						<div id="DELIVERY_ANCHOR_<?=$arDelivery["ID"]?>"></div>
					</label>
