...
<div class="internal_sections_list">
    ...
    <ul class="sections_list_wrapp">
        ...
        <? foreach ($arResult["SECTIONS_TREE"] as $arItem): ?>
            ...
            ?>
            <li class="item <?= ($arItem["SELECTED"] ? "cur" : "") ?>" id="<?= $this->GetEditAreaId($arItem['ID']); ?>"
                data-id="<?= $arItem['ID'] ?>">
                <a href="<?= $arItem["SECTION_PAGE_URL"] ?>"
                   class="<?= ($bParent ? 'parent' : '') ?>"><span><?= $arItem["NAME"] ?></span></a>
                <? if ($bParent): ?>
                            ...
                            <ul class="child">
                                <? foreach ($arItem["SECTIONS"] as $arSection): ?>
                                    ...
                                    <? if (count($arSection["SECTIONS"])): ?>
                                        <li class="bDepth3">
                                            <a class="menu_title <?= ($arSection["SELECTED"] ? "cur" : "") ?>"
                                            ...
                                            <? foreach ($arSection["SECTIONS"] as $arSubItem): ?>
                                                ...
                                                <a class="menu_item <?= ($arSubItem["SELECTED"] ? "cur" : "") ?>"
                                                   data-id="<?= $arSubItem['ID'] ?>"
                                                   href="<?= $arSubItem["SECTION_PAGE_URL"] ?>"><?= $arSubItem["NAME"] ?></a>


                                                <? if (count($arSubItem["SECTIONS"])): ?>
                                                    ...
                                                    <? foreach ($arSubItem["SECTIONS"] as $arSubSection4): ?>
                                                        ...
                                                        <?/* foreach ($arSubSection4 as $arSubItem4): */?><!--

                                                            <?php
/*                                                            //echo "<pre>";var_dump($arSubItem4);echo "</pre>";
                                                            */?>
                                                        --><?/* endforeach; */?>
                                                    <? endforeach; ?>
                                                <? endif; ?>
