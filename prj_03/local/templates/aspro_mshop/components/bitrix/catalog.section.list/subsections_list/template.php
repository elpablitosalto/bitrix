...
<?if($arResult["SECTIONS"]):?>
	<div class="articles-list articles-panels sections wrap_md">
		<?foreach($arResult["SECTIONS"] as $arItem):?>
			<?
			$this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_EDIT"));
			$this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
			$noSectionsClass = 'item_no_sections';
			if ($arItem["SECTIONS"]) {
				$noSectionsClass = '';
			}
			?>

			<div class="item <?=$noSectionsClass?> iblock section_item_inner <?=$arParams["SHOW_SECTION_DESC"] === 'N' ? 'no-decr' : ''?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
				<div class="item-wrapper">
					<?if($arParams["SHOW_SECTION_LIST_PICTURES"] != "N"):?>
						<?if($arItem["PICTURE"]["SRC"]):?>
							<?$img = CFile::ResizeImageGet($arItem["PICTURE"]["ID"], array( "width" => 198, "height" => 198 ), BX_RESIZE_IMAGE_EXACT, true );?>
							<div class="left-data">
						...
						<?elseif($arItem["~PICTURE"]):?>
							<?$img = CFile::ResizeImageGet($arItem["~PICTURE"], array( "width" => 198, "height" => 198 ), BX_RESIZE_IMAGE_EXACT, true );?>
							<div class="left-data">
								<a href="<?=$arItem["SECTION_PAGE_URL"]?>" class="thumb"><img src="<?=$img["src"]?>" alt="<?=($arItem["PICTURE"]["ALT"] ? $arItem["PICTURE"]["ALT"] : $arItem["NAME"])?>" title="<?=($arItem["PICTURE"]["TITLE"] ? $arItem["PICTURE"]["TITLE"] : $arItem["NAME"])?>" /></a>
							</div>
						<?else:?>
