	...
	foreach ($productSlider as $i => $arImage) {
		$productSlider[$i] = array_merge(
			...
			array(
				...
				"SMALL" => CFile::ResizeImageGet($arImage["ID"], array("width" => 500, "height" => 570), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])) {
	...
	if ('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS']) {
		foreach ($arResult['OFFERS'] as &$arOffer) {
			...
			while ($arStoreInfo = $rsStoreProduct->fetch()) {
				$arOffer["STORES"][$arStoreInfo["STORE_ID"]] = $arStoreInfo;
			}
			//
	...
	//
	uasort($arResult["OFFERS"], function ($element1, $element2) {
		if ($element1["PROPERTIES"]["RAZMER"]["VALUE"] > $element2["PROPERTIES"]["RAZMER"]["VALUE"]) return 1;
		if ($element1["PROPERTIES"]["RAZMER"]["VALUE"] < $element2["PROPERTIES"]["RAZMER"]["VALUE"]) return -1;
		if ($element1["PROPERTIES"]["RAZMER"]["VALUE"] == $element2["PROPERTIES"]["RAZMER"]["VALUE"]) return 0;
	});
}
...
/*brand item*/
$arBrand = array();
if (!empty($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && !empty($arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"])) {
...
// 27068
$arResult["LINKED_PRODUCTS"] = productColors::getLinkedProducts($arResult);

// Минимальное количество в упаковке -->
$arResult['MINIMUM_QUANTITY_PER_PACKAGE'] = 0;
if (!empty($arResult['PROPERTIES']['CML2_TRAITS']['VALUE'])) {
	foreach ($arResult['PROPERTIES']['CML2_TRAITS']['VALUE'] as $key => $val) {
		if ($arResult['PROPERTIES']['CML2_TRAITS']['DESCRIPTION'][$key] == 'Минимальная упаковка') {
			$arResult['MINIMUM_QUANTITY_PER_PACKAGE'] = $val;
			break;
		}
	}
}
...
$arResult['INDIVISIBLE_QUANTITY_IN_PACKAGE'] = 0;
//echo 'VALUE:';
//vardump($arResult['PROPERTIES']['CML2_TRAITS']);
if (!empty($arResult['PROPERTIES']['CML2_TRAITS']['VALUE'])) {
	foreach ($arResult['PROPERTIES']['CML2_TRAITS']['VALUE'] as $key => $val) {
		if ($arResult['PROPERTIES']['CML2_TRAITS']['DESCRIPTION'][$key] == 'НеделимаяУпаковка') {
			$arResult['INDIVISIBLE_QUANTITY_IN_PACKAGE'] = $val;
			break;
		}
	}
}
...
// Стартовое значение для количества -->
$arResult['START_QUANTITY'] = 0;
if (intval($arResult['INDIVISIBLE_QUANTITY_IN_PACKAGE']) > 0) {
	$arResult['START_QUANTITY'] = $arResult['INDIVISIBLE_QUANTITY_IN_PACKAGE'];
} else if (intval($arResult['MINIMUM_QUANTITY_PER_PACKAGE']) > 0) {
	$arResult['START_QUANTITY'] = $arResult['MINIMUM_QUANTITY_PER_PACKAGE'];
}
// <-- Стартовое значение для количества

// Свойства для вывода в карточке товара -->
$arResult['DISPLAY_PROPERTIES_LIST'] = \First\Catalog::getPropsForProductDetail('exclude');
// <-- Свойства для вывода в карточке товара


// -->
$arResult['SWP_PRICE'] = \First\Catalog::getSwpPrice();
// <--
