...
$(document).ready(function () {
	$(".size-group__table .counter_block.counter_block_slim .plus").live("click", function () {
		if (!$(this).parent().find('.js-size-table-quantity[disabled]').length) {
			if ($(this).parent().data("offers") != "Y") {
				var isDetailSKU2 = $(this).parents('.counter_block_wr').length;
				input = $(this).parents(".counter_block").find("input[type=text]")
				tmp_ratio = !isDetailSKU2 ? $(this).parents(".counter_wrapp").find(".size-group-to-cart").data('ratio') : $(this).parents('tr').first().find("td.buy .size-group-to-cart").data('ratio'),
					isDblQuantity = !isDetailSKU2 ? $(this).parents(".counter_wrapp").find(".size-group-to-cart").data('float_ratio') : $(this).parents('tr').first().find("td.buy .size-group-to-cart").data('float_ratio'),
				...
				var step = ratio;
				var indivisibleQuantity = Number(input.data('indivisible-quantity'));

				// Неделимое количество в упаковке -->
				if (indivisibleQuantity > 0) {
					step = indivisibleQuantity;
				}
				// <--
	...
	$(".size-group__table .counter_block.counter_block_slim .minus").live("click", function () {
		if (!$(this).parent().find('.js-size-table-quantity[disabled]').length) {
			if ($(this).parent().data("offers") != "Y") {
				var isDetailSKU2 = $(this).parents('.counter_block_wr').length;
				input = $(this).parents(".counter_block").find("input[type=text]")
				...
				// Неделимое количество в упаковке -->
				if (indivisibleQuantity > 0) {
					if (curValue < indivisibleQuantity) {
						curValue = indivisibleQuantity;
					}
				}
				// <-- 
	...
	$('.size-group__table .counter_block.counter_block_slim input[type=text]').numeric({ allow: "." });
	$('.size-group__table .counter_block.counter_block_slim input[type=text]').live('focus', function () {
		$(this).addClass('focus');
		$(this).val("");
	})
	$('.size-group__table .counter_block.counter_block_slim input[type=text]').live('blur', function () {
		$(this).removeClass('focus');
	})
	$('.size-group__table .counter_block.counter_block_slim input[type=text]').live('change', function (e) {
		e.preventDefault();
		if (!$(this).parents('.basket_wrapp').length) {
			var val = $(this).val(),
				tmp_ratio = $(this).parents(".counter_wrapp").find(".size-group-to-cart").data('ratio') ? $(this).parents(".counter_wrapp").find(".size-group-to-cart").data('ratio') : $(this).parents('tr').first().find("td.buy .size-group-to-cart").data('ratio'),
				isDblQuantity = $(this).parents(".counter_wrapp").find(".size-group-to-cart").data('float_ratio') ? $(this).parents(".counter_wrapp").find(".size-group-to-cart").data('float_ratio') : $(this).parents('tr').first().find("td.buy .size-group-to-cart").data('float_ratio'),
				ratio = (isDblQuantity ? parseFloat(tmp_ratio) : parseInt(tmp_ratio, 10)),
				diff = val % ratio;
			...
			// Неделимое количество в упаковке -->
			var indivisibleQuantity = Number($(this).data('indivisible-quantity'));
			if (indivisibleQuantity > 0) {
				var remains = val % indivisibleQuantity;
				if (remains > 0) {
					var whole = parseInt(val / indivisibleQuantity);
					whole++;
					if (whole == 0) {
						whole = 1;
					}
					//alert(whole);
					val = whole * indivisibleQuantity;
				}
			}
			// <--

			$(this).parents('.counter_block').parent().parent().find('.size-group-to-cart').attr('data-quantity', val);
			$(this).parents('.counter_block').parent().parent().find('.one_click').attr('data-quantity', val);
			$(this).val(val);

			if (val > 0) {
				$(this).parents(".size-table__row").addClass('size-table__row_state_checked');
				$(this).parents(".size-table__row").find('.js-size-table-checkbox').prop('checked', true);
			} else {
				$(this).parents(".size-table__row").removeClass('size-table__row_state_checked');
				$(this).parents(".size-table__row").find('.js-size-table-checkbox').prop('checked', false);
			}
	...
	(function sizeTable() {
		...
		var qtyInputSelector = '.js-size-table-quantity';
		$('html').on('change', checkboxSelector, function () {
			var $trigger = $(this),
				$container = $trigger.closest('.size-table__row');
			let price = parseFloat($container.find(".size-table__cell[data-price]").data("price"));
			let qty = parseInt($container.find("input[name='quantity']").val());
			if (this.checked) {
				$container.addClass('size-table__row_state_checked');
			} else {
				$container.removeClass('size-table__row_state_checked');
				$container.find("input[name='quantity']").val(0);
			}
			calculateTotalValues();
		});

		$('html').on('change', qtyInputSelector, function () {
			calculateTotalValues();
		});
	})();

	$(".size-group-to-cart").click(function (e) {
		e.preventDefault();
		let $button = $(this);
		$button.attr("disabled", true);
		let addData = [];
		let qtyErrors = [];
		$('.size-table__body .size-table__row').each(function () {
			let $container = $(this);
			let price = parseFloat($container.find(".size-table__cell[data-price]").data("price"));
			let qty = parseInt($container.find("input[name='quantity']").val());
			if ($container.hasClass('size-table__row_state_checked')) {
				if (qty > 0) {
					addData.push({
						'quantity': qty,
						'iblockID': $button.data("iblockid"),
						'item': $container.data("item"),
					});
				} else {
					qtyErrors.push($container.find("input[name='quantity']"));
				}
			}
			$container.find("input[name='quantity']").css("border-color", "rgb(229, 229, 229)");
		});
		if (addData.length > 0 && qtyErrors.length == 0) {
			if (BX) {
				BX.showWait();
			}
			$.ajax({
				type: "POST",
				url: arMShopOptions['SITE_DIR'] + "ajax/sizeGroupItems.php",
				data: { data: addData },
				dataType: "json",
				success: function (data) {
					getActualBasket($button.data("iblockID"));
					if (data !== null) {
						if ("STATUS" in data) {
							if (data.MESSAGE_EXT === null)
								data.MESSAGE_EXT = '';
							if (data.STATUS === 'OK') {
								addData.forEach(function (element, i, arr) {
									addBasketCounter(element.item);
								});
								...
								// 41973
								$(".size-table__body .js-size-table-quantity").val(0).trigger("change");
								if (BX) {
									BX.closeWait();
								}
								//
		...
		} else if (qtyErrors.length > 0) {
			$.fancybox({ content: "<div style='padding: 10px 25px;'><div class='size-group__error_message'>Заполните количество товара</div><button class='big_btn size-group-to-cart button' onclick='$.fancybox.close();'>Закрыть</button></div>" });
			qtyErrors.forEach(function (value, index) {
				qtyErrors[index].css("border-color", "red");
			});
			$button.attr("disabled", false);
		} else {
			$button.attr("disabled", false);
	...
	$('.js-size-group-trigger').on('click', function () {
		$(this).parents('.size-group').find('.size-table__body .size-table__row.js-limit').toggleClass('hidden');
		$(this).closest('.size-group').toggleClass('size-group_state_open');
		return false;
	});
});

function setSmallWholesalePackaging() {
	var startQuantityNum = Number($('.js_minimum_quantity_per_package').val());
	var cost = 0;
	if (startQuantityNum > 0) {
		var i = 0;
		var priceSwp = Number($('.js_swp_price').val());
		$('.js_total_qty_swp').html(String(i));
		$('.js_swp_wrapper').css('display', 'none');
		$('.js-size-table-checkbox').each(function () {
			if ($(this).is(':checked')) {
				var itemId = $(this).data('item');
				var qty_str = $('input[data-item="' + itemId + '"]').val();
				var max_qty_str = $(this).data('max-qty');
				var qty = Number(qty_str);
				var max_qty = Number(max_qty_str);

				var needSwp = false;

				//console.log(max_qty);

				// Добавлять ли мелкооптовую упаковку -->
				if (qty > 0) {
					if (qty <= max_qty) {
						needSwp = false;
					} else {
						if (qty > max_qty && qty < startQuantityNum) {
							...
							if (qty > startQuantityNum && qty <= (startQuantityNum + max_qty)) {
								...
								var whole_digit = parseInt(qty / startQuantityNum);
								var whole_digit_up = Math.ceil(qty / startQuantityNum);
				...
				if (needSwp) {
					i++;
					$('.js_total_qty_swp').html(String(i));
					$('.js_swp_wrapper').css('display', 'block');
					cost += i * priceSwp;
				}
			}
		});
...
function calculateTotalValues() {
	...
	var swpCost = setSmallWholesalePackaging();
	//console.log(swpCost);
	...
	$(".offer-final .offer-final__prop-value-qty").html(totalQuantity);
	$(".offer-final .offer-final__prop-value-price").html(totalPrice);
}
