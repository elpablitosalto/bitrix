				...
				<? if ($arResult['ITEMS_BY_SECTIONS'][$SID]): ?>
					<? if (count($arResult['SECTIONS']) > 1): ?>
						...
						<div class="staff_section" id="<?= $this->GetEditAreaId($SID) ?>">
							<div class="staff_section_title">
								<h4><a rel="nofollow" href=""><?= $SName ?></a><span class="slide opener_icon no_bg"><i></i></span></h4>
							</div>
						<? endif; ?>
								...
								<div class="item <?= ($bShowImage ? '' : 'wi') ?>" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
									...
									<div class="info">
										...
										<? if (strlen($arItem["DISPLAY_PROPERTIES"]["PHONE"]["VALUE"])): ?>
											<div class="phone">
												<div><?= GetMessage('PHONE') ?></div><?= $arItem["DISPLAY_PROPERTIES"]["PHONE"]["VALUE"] ?>
											</div>
										<? endif; ?>
										<? if (strlen($arItem["DISPLAY_PROPERTIES"]["EMAIL"]["VALUE"])): ?>
											<div class="email">
												<div><?= GetMessage('EMAIL') ?></div><a rel="nofollow" href="mailto:<?= $arItem["DISPLAY_PROPERTIES"]["EMAIL"]["VALUE"] ?>"><?= $arItem["DISPLAY_PROPERTIES"]["EMAIL"]["VALUE"] ?></a>
											</div>
										<? endif; ?>
									</div>
								</div>
