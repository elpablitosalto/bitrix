...
$(document).ready(function () {
	...
	*/
	...
	$(document).on('click', '#headerfixed_mobile ul.menu.adaptive .menu_opener', function () {
		$(this).parents(".menu.adaptive").toggleClass("opened"); console.log(this);
		$("#headerfixed_mobile ul.menu.full").toggleClass("opened").slideToggle(200);
	});
	/*menu open/close end*/
	...
	$('.counter_block:not(.basket) input[type=text]').numeric({ allow: "." });
	$('.counter_block:not(.basket) input[type=text]').live('focus', function () {
		$(this).addClass('focus');
	})
	$('.counter_block:not(.basket) input[type=text]').live('blur', function () {
		$(this).removeClass('focus');
	})
	$('.counter_block:not(.basket) input[type=text]').live('change', function (e) {
		e.preventDefault();
		if (!$(this).parents('.basket_wrapp').length) {
			var $input = $(this),
				val = $input.val(),
				tmp_ratio = $input.parents(".counter_wrapp").find(".to-cart").data('ratio') ? $input.parents(".counter_wrapp").find(".to-cart").data('ratio') : $input.parents('tr').first().find("td.buy .to-cart").data('ratio'),
				isDblQuantity = $input.parents(".counter_wrapp").find(".to-cart").data('float_ratio') ? $input.parents(".counter_wrapp").find(".to-cart").data('float_ratio') : $input.parents('tr').first().find("td.buy .to-cart").data('float_ratio'),
				ratio = (isDblQuantity ? parseFloat(tmp_ratio) : parseInt(tmp_ratio, 10)),
				diff = val % ratio;
			...
			if (parseFloat($input.parents(".counter_block").find(".plus").data('max')) > 0) {
				if (val > parseFloat($input.parents(".counter_block").find(".plus").data('max'))) {
					val = parseFloat($input.parents(".counter_block").find(".plus").data('max'));
					// val -= (val % ratio);
				}
			}
			...
			if ($input.data('value-check-url') && $input.parents(".counter_block").data('item')) {
				$input.attr('readonly', true);
				$.ajax({
					type: "POST",
					url: $input.data('value-check-url'),
					data: {
						productId: parseInt($input.parents(".counter_block").data('item')),
						quantity: val
					},
					dataType: "json",
					success: function (data) {
						if (data && data.MIN_QUANTITY_BUY) {
							val = data.MIN_QUANTITY_BUY;
							$input.parents('.counter_block').parent().parent().find('.to-cart').attr('data-quantity', val);
							$input.parents('.counter_block').parent().parent().find('.one_click').attr('data-quantity', val);
							$input.val(val);

							var eventdata = { type: 'change', params: { id: $input, value: val } };
							BX.onCustomEvent('onCounterProductAction', [eventdata]);
						}
						else {
							console.error($input.data('value-check-url') + ": Не удалось получить минимальное количество товара.");
						}
						let $minPackWrap = $input.parents('.buy_block').find('.min_pack');
						$minPackWrap.find('.min_pack_message').remove();
						if (data && data.MIN_PACK_VALUE_PRICE && data.MIN_PACK_VALUE_MESSAGE) {
							$minPackWrap.append('<div class="min_pack_message">' + data.MIN_PACK_VALUE_MESSAGE + ' ' + data.MIN_PACK_VALUE_PRICE + ' руб.</div>');
						}
					},
					complete: function () {
...
if (!funcDefined('orderActions')) {
	orderActions = function () {
		if (arMShopOptions["PAGES"]["ORDER_PAGE"]) {
			...
			if (arMShopOptions['COUNTERS']['USE_FULLORDER_GOALS'] !== 'N') {
				if (typeof window.reachgoalbegin === 'undefined') {
					window.reachgoalbegin = true;
