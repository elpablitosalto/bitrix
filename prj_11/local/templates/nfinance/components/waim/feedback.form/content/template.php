...
if(!empty($arParams["WEB_FORM_ID"])):?>
	<?$APPLICATION->IncludeComponent(
		...
		Array(
			...
			"NAME" => htmlspecialchars_decode($arParams["TITLE"]),
...
<div class="modal modal_role_subscribe"  id="feedback_form_<?=$arParams["FORM_ID"]?>_success">
	...
	<div class="modal__content">
		<?if(!empty($arParams["SUCCESS_TITLE"])):?>
			<div class="modal__title"><?=htmlspecialchars_decode($arParams["SUCCESS_TITLE"])?></div>
		<?endif;?>
		<div class="modal__text"><?=htmlspecialchars_decode($arParams["SUCCESS_DESCRIPTION"])?></div>
	</div>
</div>
...
<div class="modal modal_width_auto" id="feedback_form_<?=$arParams["FORM_ID"]?>_error">
	<?if(!empty($arParams["ERROR_TITLE"])):?>
		<div class="modal__header">
			<h2 class="title title_size_h4"><?=htmlspecialchars_decode($arParams["ERROR_TITLE"])?></h2>
		</div>
	<?endif;?>
	<div class="modal__content">
