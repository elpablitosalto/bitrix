...
if ($arResult["isFormNote"] != "Y") {
	...
	$formId = 'subscribe_form_' . $arResult["PARAMS"]["FORM_ID"];
	$arResult["FORM_HEADER"] = preg_replace('/action="\/[^"]*"/', 'action="trap.php"', $arResult["FORM_HEADER"]);
?>

	<?= str_replace('<form', '<form id="' . $formId . '" class="form form_layout_horizontal ' . $arResult["WEB_FORM_NAME"] . '"', $arResult["FORM_HEADER"]) ?>
	<script>
		(function() {
					...
					form.onSuccess(function(e) {
						...
						xhr.onload = function() {
							...
							if (xhr.status != 200) {
								...
								//cosole.log(`Ошибка ${xhr.status}: ${xhr.statusText}`);
								...
								if (!json.success) {
									...
									<?/*?>
									console.log('typeError = ' + json.typeError);
									console.log('Error = ' + errorStr);
									console.log('RECAPTCHA_ANSWER = ' + json.RECAPTCHA_ANSWER);
									console.log('RECAPTCHA_ANSWER_TEST = ' + json.RECAPTCHA_ANSWER_TEST);
	...
	<div class="form__main">
		<div class="form__inputs">
			...
			foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
				switch ($arQuestion['STRUCTURE'][0]['FIELD_TYPE']) {
					case "text":
			?>
						<? if (!empty($arResult["arQuestions"][$FIELD_SID]["COMMENTS"]) && $arResult["arQuestions"][$FIELD_SID]["COMMENTS"] == "FORM_TYPE") : ?>
							<input type="hidden" id="<?= $formId ?>_<?= $arResult["arForm"]["SID"] ?>_<?= $FIELD_SID ?>" name="form_<?= strtolower($arQuestion["STRUCTURE"][0]["FIELD_TYPE"]) ?>_<?= $arQuestion["STRUCTURE"][0]["ID"] ?>" value="<?= $arResult["PARAMS"]["FORM_TYPE"] ?>" />
						<? endif; ?>
					<?
						break;
					case "email":
					?>
						<div class="form__line">
							<!-- begin .form-control-->
							<div class="form-control <? if (is_array($arResult["FORM_ERRORS"]) && array_key_exists($FIELD_SID, $arResult['FORM_ERRORS'])) : ?>form-control__input_state_invalid<? endif; ?>">
								<label class="form-control__holder">
									<span class="form-control__field">
										<input type="email" class="form-control__input" id="<?= $formId ?>_<?= $arResult["arForm"]["SID"] ?>_<?= $FIELD_SID ?>" <? if ($arQuestion["REQUIRED"] == "Y") : ?>required="required" <? endif; ?> name="form_<?= strtolower($arQuestion["STRUCTURE"][0]["FIELD_TYPE"]) ?>_<?= $arQuestion["STRUCTURE"][0]["ID"] ?>" <? if (!empty($arResult["PARAMS"]["PLACEHOLDER"])) : ?>placeholder="<?= $arResult["PARAMS"]["PLACEHOLDER"] ?>" <? endif; ?> <? if (
																																																																																																																								!empty($arResult["arrVALUES"]["form_" . strtolower($arQuestion["STRUCTURE"][0]["FIELD_TYPE"]) . "_" . $arQuestion["STRUCTURE"][0]["ID"]])
																																																																																																																							) : ?> value="<?= $arResult["arrVALUES"]["form_" . strtolower($arQuestion["STRUCTURE"][0]["FIELD_TYPE"]) . "_" . $arQuestion["STRUCTURE"][0]["ID"]] ?>" <? endif; ?> />
		...
		<div class="form__controls">
			<div class="form__control">
				<!-- begin .button-->
				<button class="button button_width_full button_style_gray" data-type="submit" type="submit" name="web_form_submit" disabled value="<?= htmlspecialcharsbx(trim($arResult["arForm"]["BUTTON"]) == '' ? GetMessage("FORM_ADD") : $arResult["arForm"]["BUTTON"]); ?>">
