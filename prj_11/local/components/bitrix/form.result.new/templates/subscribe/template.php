...
?>
	...
	<?= str_replace('<form', '<form id="'.$formId.'" class="attention-panel__fields'.$arResult["WEB_FORM_NAME"].'"', $arResult["FORM_HEADER"]) ?>
		<script>
			(function() {
							...
							xhr.onload = function () {
								if (xhr.status != 200) {
									...
									if (!json.success) {
										...
										var messageHolder = formNode.getElementsByClassName("form__message");
										if(messageHolder.length) {
											messageHolder[0].innerHTML = errorStr;
										}
		...
		<div class="attention-panel__fields">
						...
						case "text":
							...
							<?if(!empty($arResult["arQuestions"][$FIELD_SID]["COMMENTS"]) && $arResult["arQuestions"][$FIELD_SID]["COMMENTS"] == "FORM_TYPE"):?>
								<input
									...
									value="<?=$arResult["PARAMS"]["FORM_TYPE"]?>"/>
							<?endif;?>
							<?
							break;
						case "email":
							...
							<div class="attention-panel__field">
								<!-- begin .form-control-->
								<div class="form-control form-control_size_md form-control_style_outline <?if (is_array($arResult["FORM_ERRORS"]) && array_key_exists($FIELD_SID, $arResult['FORM_ERRORS'])):?>form-control__input_state_invalid<?endif;?>">
			...
			<div class="attention-panel__controls">
				<div class="attention-panel__control">
					<!-- begin .button-->
					<button
						class="button button_width_full button_size_md button_style_dark-grey"
						data-type="submit"
						type="submit"
		...
		<div class="attention-panel__checkbox">
				<!-- begin .check-elem-->
				<label class="check-elem check-elem_text-size_s check-elem_style_light">
