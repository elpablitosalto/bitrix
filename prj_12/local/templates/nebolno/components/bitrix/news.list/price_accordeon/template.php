...
<?if (count($arResult['ITEMS']) > 0):?>
	<div class="nb-section__body">
		<ul class="nb-price-list">
			<?
			$currentAccordeonText = $arParams['ACCORDEON_DATA'][0];
			?>
			<li class="nb-price-item nb-price-item_opened">
				<div class="nb-price-item__header">
					<p class="nb-price-item__title"><?=$currentAccordeonText;?></p>
				</div>
				<div class="nb-price-item__body">
					<div class="nb-price-table">
						<?foreach($arResult["ITEMS"] as $key => $arItem):?>

							<?if (isset($arParams['ACCORDEON_DATA'][$key]) && mb_strlen($arParams['ACCORDEON_DATA'][$key]) > 0 && $arParams['ACCORDEON_DATA'][$key] != $currentAccordeonText):?>
								<?
								$currentAccordeonText = $arParams['ACCORDEON_DATA'][$key];
								?>
										</div>
									</div>
								</li>
								<li class="nb-price-item">
									<div class="nb-price-item__header">
										<p class="nb-price-item__title"><?=$currentAccordeonText?></p>
									</div>
									<div class="nb-price-item__body">
		...
		<div class="nb-price-note">
			...
			<a class="nb-btn nb-btn_light nb-btn_shadow" href="/prices/">Показать все цены</a>
		</div>
	</div>
<?endif;?>
