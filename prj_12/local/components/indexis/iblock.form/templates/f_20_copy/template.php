...
<section class="nb-f3-section" id="<?= $arParams['BLOCK_AREA_ID'] ?>">
	<div class="nb-f3">
		<div class="container" id="<?= $arParams['EDIT_AREA_ID'] ?>">
			<h2 class="nb-f3__title">Оставить отзыв</h2>
			
			<form data-form-name="<?=htmlspecialchars($arResult['IBLOCK']['NAME'])?>" class="nb-form nb-consultation-form nb-f20-form" action="<?= $arResult['ACTION'] ?>" method="POST" <? if (!empty($arParams['FORM_CODE'])) : ?> data-form-code="<?= $arParams['FORM_CODE'] ?>" <? endif; ?>>
				<div class="nb-form__body">
					...
					<div class="nb-form__actions">
						<button class="nb-btn nb-form__submit" type="submit">Отправить</button>
					</div>
				</div>
