$(function () {
  ...
  var showError = function ($form, message) {
    ...
    $formContent.prepend(
      '<div class="form-error-message"><p>' + message + '</p></div>'
    );
  };
    ...
    .each(function () {
      $(this).validate({
        ...
        submitHandler: function (form) {
          ...
          if (
            typeof Comagic !== 'undefined' &&
            typeof Comagic.addOfflineRequest !== 'undefined'
          ) {
            Comagic.addOfflineRequest({
              name: $form.find('input[name="NAME"]').length
                ? $form.find('input[name="NAME"]').val()
                : '',
              email: $form.find('input[name="EMAIL"]').length
                ? $form.find('input[name="EMAIL"]').val()
                : '',
              phone: $form.find('input[name="PHONE"]').length
                ? $form.find('input[name="PHONE"]').val()
                : '',
              message: $form.find('input[name="MESSAGE"]').length
          ...
          $.ajax({
            ...
            success: function (result) {
              if (typeof result.SUCCESS !== 'undefined') {
                ...
                $('#modal-feedback-success').fadeIn(300);
                $form.trigger('reset');
