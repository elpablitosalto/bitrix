	...
	<div class="nb-modal__dialog" id="<?= $arParams['BLOCK_AREA_ID'] ?>">
		...
		<div class="nb-modal__body" id="<?= $arParams['EDIT_AREA_ID'] ?>">
			...
			<div class="nb-modal-call__form">
				<form data-form-name="<?=htmlspecialchars($arResult['IBLOCK']['NAME'])?>" class="nb-form nb-modal-call-form" action="<?= $arResult['ACTION'] ?>" method="POST" <? if (!empty($arParams['FORM_CODE'])) : ?> data-form-code-doctor-popup="<?= $arParams['FORM_CODE'] ?>" <? endif; ?>>
							...
							foreach ($arResult['FIELDS'] as $arField) : ?>
								...
								switch ($arField['PROPERTY_TYPE']) {
									...
									default:
										...
										?>
											<input type="hidden" id="<?= $arField['CODE'] ?>" name="<?= $arField['CODE'] ?>" value="<? if (isset($arParams['DEFAULT_' . $arField['CODE']])) : ?><?= $arParams['DEFAULT_' . $arField['CODE']] ?><? endif; ?>">
										<?
										} else {
										?>
											<div class="nb-form-field">
				...
				<p class="nb-modal-call__agreement">Нажимая на&nbsp;кнопку «Записаться» вы соглашаетесь на&nbsp;обработку персональных данных. <br><a href="/dokumenty/#block-4097" target="_blank">Политика конфиденциальности</a></p>
			</div>
		</div>
	</div>
</div>