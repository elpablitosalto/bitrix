...
<div id="<?= $arParams['BLOCK_AREA_ID'] ?>">
	<section class="nb-f1-section" id="<?= $arParams['EDIT_AREA_ID'] ?>">
		<picture class="nb-f1-section__bg">
			<img src="<?= SITE_TEMPLATE_PATH ?>/img/content/consultation/consultation-section-bg.jpg" alt="" />
		</picture>
		<div class="nb-f1">
			<div class="container">
				<div class="nb-f1__desc">
					<p>Получить <span class="nb-f1__desc_desktop">более</span> подробную информацию: <span class="nb-f1__phone-wrapper"><a class="nb-f1__phone" href="tel:+74957836606">+7 (495) 783-66-06</a></span></p>
					<p class="color_green"><span>Или заполните форму,</span> <span>и&nbsp;наш специалист перезвонит вам</span></p>
				</div>
				<form data-form-name="<?=htmlspecialchars($arResult['IBLOCK']['NAME'])?>" class="nb-form nb-consultation-form nb-f1-form" action="<?= $arResult['ACTION'] ?>" method="POST" <? if (!empty($arParams['FORM_CODE'])) : ?> data-form-code="<?= $arParams['FORM_CODE'] ?>" <? endif; ?>>
							...
							<? foreach ($arResult['FIELDS'] as $arField) : ?>
								...
								?>
									...
									default:
										...
										?>
											<div class="nb-form-field">
												<input id="<?=toLower($arParams['FORM_CODE'] . '_' . $arField['CODE'])?>" class="form-control simple" placeholder="<? if ($arField['CODE'] == 'PHONE') : ?><?= Loc::getMessage('FS_FIELD_PHONE_PLACEHOLDER') ?><? else : ?><?= $arField['NAME'] ?><? endif; ?>" type="<?= $fieldType ?>" name="<?= $arField['CODE']; ?>" <?= $strValidateAttrs ?>>
					...
					<div class="nb-form__footer">
						<p class="nb-form__agreement">
							Нажимая на кнопку, вы соглашаетесь на обработку персональных данных <span class="nb-form__agreement-rules"><a href="/dokumenty/" target="_blank">политика конфиденциальности</a></span>
