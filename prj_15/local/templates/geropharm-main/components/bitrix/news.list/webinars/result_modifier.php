...
foreach ($arResult["ITEMS"] as &$arItem) {
    $ar = array();
    if (is_array($arItem['DISPLAY_PROPERTIES']['SPEAKER']['VALUE'])) {
        $ar = $arItem['DISPLAY_PROPERTIES']['SPEAKER']['VALUE'];
    } else {
        if (intval($arItem['DISPLAY_PROPERTIES']['SPEAKER']['VALUE']) > 0) {
            $ar = array($arItem['DISPLAY_PROPERTIES']['SPEAKER']['VALUE']);
        }
    }
...
if ($arParams['USER_AUTHORIZED'] == 'Y') {
    if (!empty($arParams['USER_ORDERS'])) {
        $arResult['ORDERS'] = $arParams['USER_ORDERS'];
    } else {
        $deal = new Deal();
...
// Какую кнопку показать -->
foreach ($arResult["ITEMS"] as &$arItem) {
    $arItem['BUTTON_TYPE'] = 'SEE';
    if (
        (
            !empty($arItem['DISPLAY_PROPERTIES']['BUY_LINK']['VALUE'])
            && $arParams['USER_AUTHORIZED'] == 'Y'
            && !isset($arResult['ORDERS'][$arItem['ID']])
        ) || (
            !empty($arItem['DISPLAY_PROPERTIES']['BUY_LINK']['VALUE'])
            && $arParams['USER_AUTHORIZED'] == 'N'
        )
    ) {
        $arItem['BUTTON_TYPE'] = 'BUY';
    }
}
// <-- Какую кнопку показать
...
if ($arParams['OG']['SET'] == 'Y') {
    foreach ($arResult["ITEMS"] as &$arItem) {
        ...
        $arResult['OG']['OG_IMAGE'] = $arItem['PICTURE']['SRC'];
...
$this->__component->SetResultCacheKeys(array("OG"));
