...
<script>
    <?
	$url = ((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $arResult["DETAIL_PAGE_URL"];
    ...
    ?>
    $(function() {
        mindboxViewMaterial({'specialization':<?echo CUtil::PhpToJSObject($arResult["DISPLAY_PROPERTIES"]["SPECIALITY"]["DISPLAY_VALUE"]);?>,'link':'<?=$url?>', 'name':'<?=$arResult["NAME"]?>'});
    });
</script>
...
if ($arParams['OG']['SET'] == 'Y') {
    $arResultFunc = CMarkingOG::getSetGlobalData(array(
        "OG_TITLE" => $arResult['OG']['OG_TITLE'],
        "OG_URL" => $arResult['OG']['OG_URL'],
        "OG_DESCRIPTION" => $arResult['OG']['OG_DESCRIPTION'],
