...
<? if (!empty($arResult['arFilterResult'])) { ?>
    ...
    $GLOBALS['arrFilterSeeAlso']['!PROPERTY_FILE'] = false;
    ?>
...
<script>
    $(function() {
        mindboxViewPageWebinar(<?= $arResult["EX_ID"] ?>);
    });
</script>

<? if ($arResult['SHOW_WAIT_PAYMENT'] == 'Y' && $arParams['USER_AUTHORIZED'] == 'Y') { ?>
    <script>
        $(function() {
            initCheckMaterialPurchased(<?= $arResult['ID']; ?>);
        });
    </script>
<? } ?>
