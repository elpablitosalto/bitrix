function SelectObjectFieldWidget() {
    ...
    this.openSearchModal = function() {
    	...
    	if( ! this.searchModal ) {
			...
			$searchBlock = $('<div class="input-group margin-bottom-10"><input placeholder="' + placeholderValue + '" type="text" class="search-str form-control"><span class="input-group-btn"><button class="btn btn-primary search-btn" type="submit">' + buttonValue + '</button></span></div>');
			...
			$clearBtn = $('<button class="pull-left margin-left-5 btn btn-link">' + Yii.t('common', 'Сбросить') + ' </button>')
    ...
    this.updateSelectedView = function() {
    	...
    	if( ! this.preSelectedIds.length || this.preSelectedIds.length == 0 ) {
			...
			ajaxCall('/pl/logic/context/object-titles', { context: this.field.context, ids: this.preSelectedIds }, {}, function (response) {
				for ( var key in response.data.ids ) {
					...
					var $el = $("<div class='margin-bottom-5'><span>" + title + " (<a href='" + url + "' target='_blank'>id:" + id + "</a>)</span></div>")
    ...
    this.loadData = function( element ) {
    	...
    	if( ! this.selectedIds.length ) {
    		...
    		ajaxCall('/pl/logic/context/object-titles', { context: this.field.context, ids: this.selectedIds }, {}, function (response) {
    			element.empty();
				for ( var key in response.data.ids ) {
					var id = response.data.ids[key]
					var title = response.data.titles[id];
