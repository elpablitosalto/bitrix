			...
			<div class="row flex-lg-row-reverse">
				<div class="col-lg-6">
					<? if (isset($arItem["PREVIEW_PICTURE"]["SRC"])) { ?>
						<div class="dp-courses-item__img">
							<a class="dp-courses-item__img-link" target="_blank" href="<?= $arItem['PROPERTIES']['BUY_LINK']['VALUE'] ?>">
								<img loading="lazy"
                ...
                <div class="col-lg-6">
                    <div class="dp-courses-item__caption">
                        <div class="dp-courses-item__tags"><span class="dp-courses-item__tag dp-courses-item__category">курс</span></div>
						<a class="dp-courses-item__title-link" target="_blank" href="<?= $arItem['PROPERTIES']['BUY_LINK']['VALUE'] ?>">
                        ...
                        <div class="dp-courses-item__meta">
							...
							<?if((isset($arItem["DISPLAY_PROPERTIES"]["DATE_START"]["VALUE"]) && mb_strlen($arItem["DISPLAY_PROPERTIES"]["DATE_START"]["VALUE"]))
								...
								<?if(isset($arItem["DISPLAY_PROPERTIES"]["DATE_START"]["VALUE"]) && mb_strlen($arItem["DISPLAY_PROPERTIES"]["DATE_START"]["VALUE"])){?>
									<?=$arItem["DISPLAY_PROPERTIES"]["DATE_START"]["VALUE"] ?>
								<?}?>
									<?if((isset($arItem["DISPLAY_PROPERTIES"]["DATE_START"]["VALUE"]) && mb_strlen($arItem["DISPLAY_PROPERTIES"]["DATE_START"]["VALUE"]))
										&& (isset($arItem["DISPLAY_PROPERTIES"]["DATE_END"]["VALUE"]) && mb_strlen($arItem["DISPLAY_PROPERTIES"]["DATE_END"]["VALUE"]))
									){?>
									...
									<?if(isset($arItem["DISPLAY_PROPERTIES"]["DATE_END"]["VALUE"]) && mb_strlen($arItem["DISPLAY_PROPERTIES"]["DATE_END"]["VALUE"])){?>
										<?=$arItem["DISPLAY_PROPERTIES"]["DATE_END"]["VALUE"] ?>
									<?}?>
							</span>
							<?}?>
                        </div>
						...
						<div class="dp-courses-item__actions">
							<a target="_blank" class="dp-btn dp-btn_m dp-courses-item__btn-detail" href="<?= $arItem['PROPERTIES']['BUY_LINK']['VALUE'] ?>">Подробнее</a>
						</div>
                    </div>
                </div>
            </div>
