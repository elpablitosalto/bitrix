				...
				<div class="dp-item-block  dp-slider dp-courses">
					<div class="dp-item-list">
						<? foreach ($arResult["ITEMS"] as $arItem) { ?>
							...
							$cgTargetLink = '';
							?>
							<? if ($arParams['USER_AUTHORIZED'] == 'Y' && isset($arResult['ORDERS'][$arItem['ID']])) { ?>
								<? if (is_array($arItem['PROPERTIES']['GC']['VALUE'])) { ?>
									<? foreach ($arItem['PROPERTIES']['GC']['VALUE'] as $arValues) { ?>
										<?
										if (trim(toUpper($arValues['SUB_VALUES']['GC_NAME']['~VALUE'])) == trim(toUpper($arResult['ORDERS'][$arItem['ID']]['~NAME']))) {
											$cgTargetLink = $arValues['SUB_VALUES']['GC_TARGET_LINK']['VALUE'];
										}
										?>
									<? } ?>
								<? } ?>
								...
								<div class="dp-course">
									<? if (!empty($arItem['PICTURE'])) { ?>
										<div class="dp-course__img">
											<a target="_blank" class="dp-course__img-link" href="<?if (!empty($cgTargetLink)):?><?=$cgTargetLink?><?else:?><?= $arItem['DISPLAY_PROPERTIES']['BUY_LINK']['VALUE'] ?><?endif;?>">
									...
									<div class="dp-course__caption">
										<div class="dp-course__tags">
											<span class="dp-course__tag dp-course__category">Курс</span>
											<? foreach ($arItem['DISPLAY_PROPERTIES']['THEME']['DISPLAY_VALUE'] as $key => $val) { ?>
												<span class="dp-course__tag dp-course__category"><?= $val; ?></span>
											<? } ?>
										</div>
										<a target="_blank" class="dp-course__title-link js_articles_list" data-id="<?= $arItem['ID']; ?>" href="<?if (!empty($cgTargetLink)):?><?=$cgTargetLink?><?else:?><?= $arItem['DISPLAY_PROPERTIES']['BUY_LINK']['VALUE'] ?><?endif;?>" data-iblock-id="<?= $arParams['IBLOCK_ID']; ?>">
										...
										<div class="dp-course__actions">
											<? if (isset($arResult['ORDERS'][$arItem['ID']])) { ?>
												<? if (is_array($arItem['PROPERTIES']['GC']['VALUE'])) { ?>
													<? foreach ($arItem['PROPERTIES']['GC']['VALUE'] as $arValues) { ?>
														<?
														if (trim(toUpper($arValues['SUB_VALUES']['GC_NAME']['~VALUE'])) == trim(toUpper($arResult['ORDERS'][$arItem['ID']]['~NAME']))) {
															?>
															<a class="dp-btn dp-btn_m dp-course__btn" href="<?=$arValues['SUB_VALUES']['GC_TARGET_LINK']['VALUE']?>">Перейти к курсу</a>
															<?
														}
														?>
													<? } ?>
											...
											<? } else { ?>
												<? if (!empty($arItem['DISPLAY_PROPERTIES']['EXTERNAL_URL']['VALUE'])) { ?>
													<a class="dp-btn dp-btn_m dp-course__btn-free-lesson" target="_blank" href="<?= $arItem['DISPLAY_PROPERTIES']['EXTERNAL_URL']['VALUE']; ?>">Пройти пробный урок бесплатно</a>
												<? } ?>
												<? if (!empty($arItem['DISPLAY_PROPERTIES']['BUY_LINK']['VALUE'])) { ?>
													<a target="_blank" class="dp-btn dp-btn_m dp-course__btn-detail" href="<?= $arItem['DISPLAY_PROPERTIES']['BUY_LINK']['VALUE'] ?>">Подробнее</a>
												<? } ?>
											<? } ?>
										</div>
									</div>
