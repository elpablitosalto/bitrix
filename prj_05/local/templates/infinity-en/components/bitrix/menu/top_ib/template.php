...
<? if (!empty($arResult)): ?>
	<nav class="nav nav_layout_horizontal nav_type_primary js-overflowing-nav">
		<ul class="nav__list">
			<?
			$previousLevel = 0;
			foreach ($arResult as $arItem): ?>
				...
				<? if ($arItem["IS_PARENT"]) { ?>
					<li class="nav__item">
						...
						$link = "#";
						?>
						<a class="nav__link <? if ($arItem["SELECTED"]) { ?>nav__link_state_active<? } ?>" href="<?= $link; ?>" onclick="return false;"><?= $arItem["TEXT"] ?></a>
						<div class="nav__sub">
							<ul class="nav__list">
							<? } else { ?>
								<li class="nav__item">
