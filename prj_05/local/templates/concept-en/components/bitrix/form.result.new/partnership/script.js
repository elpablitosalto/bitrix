(function validationAsk() {
    ...
    (function formValidationAsk() {
        ...
        const formSelector = '#js_partnership_form';
        ...
        function init() {
            validateFormGroup(
                ...
                function (e, form, validation) {
                    ...
                    setTimeout(function () {
                        sendFormAjax_partnership(formObject);
                    }, 500);
                },
            );
        }
...
function sendFormAjax_partnership(formObject) {
    const form = document.getElementById('js_partnership_form'); // id формы
    //let url = form.getAttribute("action");
    let url = document.getElementById('js_template_path_partnership').value;
    ...
    BX.ajax({
        ...
        onsuccess: function (data) { // в случаи успеха, выполняем действия
            ...
            if (jsonObject.SUCCESS == 'Y') {
                ...
                document.getElementById('js_partnership_form_messages').style.display = "none";

                scrollToAnchorClass('js_page_scroll_after_form_send', 200);
...
function showError_ask(error) {
    document.getElementById('js_partnership_form_messages').style.display = "block";
    document.getElementById('js_partnership_form_error_message').style.display = "block";
    document.getElementById('js_partnership_form_error_message').innerHTML = error;
    document.getElementById('js_partnership_form_send_button').classList.remove("button_state_loading");
    document.getElementById('js_partnership_form_send_button').removeAttribute("disabled");
    scrollToAnchorClass('js_page_scroll_after_form_send', 200);
}
