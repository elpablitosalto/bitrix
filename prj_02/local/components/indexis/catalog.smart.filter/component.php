...
if ($this->StartResultCache(false, array('v10', $preFilter, ($arParams["CACHE_GROUPS"] ? $USER->GetGroups() : false)))) {
	...
	if (!empty($arResult["ITEMS"])) {
		if ($this->facet->isValid()) {
			...
			// Оригинал -->
			if (empty($GLOBALS[$arParams["FIRSTFILTER_NAME"]]["arrFilter"])) {
				...
				//vardump($GLOBALS[$arParams["FIRSTFILTER_NAME"]]["arPID"]);
				...
				foreach ($arElements as $arElement) {
					...
					foreach ($arResult["ITEMS"] as $PID => $arItem) {
						...
						// Доработка -->
						$arPID = $GLOBALS[$arParams["FIRSTFILTER_NAME"]]["arPID"];
						if (is_array($arPID)) {
							if (!empty($arPID) && !in_array($PID, $arPID)) {
								continue;
							}
						}
						// <-- Доработка	
			...
			// Доработка -->
			//vardump($GLOBALS[$arParams["FIRSTFILTER_NAME"]]);
			if (!empty($GLOBALS[$arParams["FIRSTFILTER_NAME"]]["arrFilter"])) {
				$_arrFilter = $GLOBALS[$arParams["FIRSTFILTER_NAME"]]["arrFilter"];
				$_arrFilter_2 = $_arrFilter;
				//vardump($_arrFilter);
				//vardump($arResult["ITEMS"]);
				foreach ($arResult["ITEMS"] as $_pid => $val) {
					//vardump($arResult["ITEMS"]);
					$arElementFilter = array(
						"IBLOCK_ID" => $this->IBLOCK_ID,
						"SUBSECTION" => $this->SECTION_ID,
					...
					// Доработка -->
					$ar_filter = array();
					foreach ($_arrFilter_2 as $_pid_2 => $_ar_filter_2) {
						if ($_pid_2 != $_pid) {
							if (!empty($_ar_filter_2) && is_array($_ar_filter_2)) {
								$arElementFilter = array_merge($_ar_filter_2, $arElementFilter);
							}
						}
					}
					//vardump($arElementFilter);
					...
					if (!empty($this->arResult["PROPERTY_ID_LIST"])) {
						$rsElements = CIBlockElement::GetPropertyValues($this->IBLOCK_ID, $arElementFilter, false, array('ID' => $this->arResult["PROPERTY_ID_LIST"]));
						while ($arElement = $rsElements->Fetch())
							$arElementsFilter[$arElement["IBLOCK_ELEMENT_ID"]] = $arElement;
					}

					if (empty($arElementsFilter)) {
						$rsElements = CIBlockElement::GetList(array('ID' => 'ASC'), $arElementFilter, false, false, array('ID', 'IBLOCK_ID'));
						while ($arElement = $rsElements->Fetch())
							$arElementsFilter[$arElement["ID"]] = array();
					}

					if (!empty($arElementsFilter) && $this->SKU_IBLOCK_ID && $arResult["SKU_PROPERTY_COUNT"] > 0) {
						$arSkuFilter = array(
							"IBLOCK_ID" => $this->SKU_IBLOCK_ID,
							"ACTIVE_DATE" => "Y",
							"ACTIVE" => "Y",
							"CHECK_PERMISSIONS" => "Y",
							"=PROPERTY_" . $this->SKU_PROPERTY_ID => array_keys($arElementsFilter),
						);
						if ($this->arParams['HIDE_NOT_AVAILABLE'] == 'Y')
							$arSkuFilter['AVAILABLE'] = 'Y';
						...
						while ($arSku = $rsElements->Fetch()) {
							foreach ($arResult["ITEMS"] as $PID => $arItem) {
								if (isset($arSku[$PID]) && $arSku[$this->SKU_PROPERTY_ID] > 0) {
									if (is_array($arSku[$PID])) {
										foreach ($arSku[$PID] as $value)
											$arElementsFilter[$arSku[$this->SKU_PROPERTY_ID]][$PID][] = $value;
									} else {
										$arElementsFilter[$arSku[$this->SKU_PROPERTY_ID]][$PID][] = $arSku[$PID];
									}
								}
							}
						}
					...
					//vardump($arElementsFilter);

					CTimeZone::Disable();
					$uniqTest = array();
					foreach ($arElementsFilter as $arElement) {
						$propertyValues = $propertyEmptyValuesCombination;
						$uniqStr = '';
						foreach ($arResult["ITEMS"] as $PID => $arItem) {
							// Доработка -->
							if (intval($_pid) > 0 && $_pid != $PID) {
								continue;
							}
							// <-- Доработка
