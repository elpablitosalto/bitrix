	...
	<div class="items-list news-list">
		...
		?>
			...
			<div class="col-sm-6 col-lg-4" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
				<? if ($show_type == 1) { ?>
					<div class="list-item news-item"><a target="_self" href="<?= $arItem["DETAIL_PAGE_URL"]; ?>" class="news-item__image">
							<picture>
						...
						<div class="h5 news-item__title"><a target="_self" href="<?= $arItem["DETAIL_PAGE_URL"]; ?>"><?= $arItem["NAME"]; ?></a></div>
				...
				<? } ?>
				<? if ($show_type == 2) { ?>
					<a target="_self" href="<?= $arItem["DETAIL_PAGE_URL"]; ?>" class="list-item news-item news-item-2">
						<div class="news-item__info">
							<span class="text-size-sm news-item__date">
								<? echo FormatDate("j F Y", MakeTimeStamp($arItem["DISPLAY_PROPERTIES"]["PUBLIC_DATE"]["VALUE"])); ?>
							</span>
				...
				<? } ?>
				<? if ($show_type == 3) { ?>
					<a target="_self" href="<?= $arItem["DETAIL_PAGE_URL"]; ?>" class="list-item news-item news-item-2 bg-orange">
						<div class="news-item__info">
							<span class="text-size-sm news-item__date">
								<? echo FormatDate("j F Y", MakeTimeStamp($arItem["DISPLAY_PROPERTIES"]["PUBLIC_DATE"]["VALUE"])); ?>
							</span>
				...
				<? if ($show_type == 4) { ?>
					<div class="list-item news-item news-item-media">
						<a target="_self" href="<?= $arItem["DETAIL_PAGE_URL"]; ?>" class="news-item__image">
							<picture>
								<img class="lazyload" src="<?= SITE_TEMPLATE_PATH ?>/images/loader.svg" data-src="<?= $arItem["PREVIEW_PICTURE"]["SRC"]; ?>" loading="lazy" alt="<?= $arItem["NAME"]; ?>" title="<?= $arItem["NAME"]; ?>" />
							</picture>
						</a>
						<div class="h5 news-item__title"><a target="_self" href="<?= $arItem["DETAIL_PAGE_URL"]; ?>"><?= $arItem["NAME"]; ?></a>
						</div>
						<div class="news-item__info">
							<span class="text-size-sm news-item__date">
								<? echo FormatDate("j F Y", MakeTimeStamp($arItem["DISPLAY_PROPERTIES"]["PUBLIC_DATE"]["VALUE"])); ?>
