...
class Indexis
{
    ...
    /**
     * Возвращает количество бонусов по цене
     */
    public static function getBonusValueByPrice($price)
    {
        return floor(($GLOBALS['arSiteConfig']['BONUS_PERCENT'] / 100) * $price);
    }
    ...
    public static function getImageFormatted($arParams)
    {
        ...
        if (/*!is_file($_SERVER["DOCUMENT_ROOT"] . $arResult['FILE_VALUE']['SRC']) && is_file( $_SERVER["DOCUMENT_ROOT"] . $noPhotoSrc )*/!isset($arPicture['SRC'])) {
        ...
        $arResult['PICTURE'] = array(
            ...
            'SOURCE_PICTURE' => $arResult['FILE_VALUE'],
        );
