...
?>
<? if ($_POST['ajaxCall'] != 'Y') { ?>
	<div class="popup order-order js_popup_order_order" id="js_popup_order_order">
	<? } ?>
	...
	<? } ?>
	<div class="js_product_on_order_form_container">
		<?
		...
		<? if ($arResult["isFormNote"] != "Y") { ?>
			<h2>Под заказ</h2>
			<p class="popup-form__text">Пришлите свой запрос, чтобы наши специалисты уточнили сроки поставки и цену «под заказ». Оставьте свои контактные данные и комментарий, мы свяжемся с вами.</p>
			<div class="order-order__form">
				<?
				foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
					if (isset($arParams[$FIELD_SID]) /*&& $arParams[$FIELD_SID]['VALUE']*/ && $arParams[$FIELD_SID]['AUTOCOMPLETE'] == 'Y') {
						$arQuestion['HTML_CODE'] = str_replace('name=', 'class="js_product_name_form_product_on_order" value="' . $arParams[$FIELD_SID]['VALUE'] . '" name=', $arQuestion['HTML_CODE']);
					}
					//vardump($arQuestion);
					if ($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden') {
						...
						switch ($FIELD_SID) {
							case 'NAME':
								$form__input_class = 'order-order__name';
								break;
							case 'PHONE':
								$form__input_class = '';
								break;
							case 'EMAIL':
								//$form__input_class = 'order-wholesale__name';
								break;
							case 'MESSAGE':
								$form__input_class = 'order-order__text';
								break;
							case 'FILE':
								$form__input_class = 'order-order__upload';
								break;
						}
				?>
						<div class="form__input <?= $form__input_class; ?>">
							...
							<? } else if ($type == "file") { ?>
								<label class="order-order__file" for="order-order__file">
									<div class="meeting-form__placeholder">Загрузить файл</div>
									<input class="order__input" id="order-order__file" type="file" name="<?= $name ?>" placeholder="Загрузить файл">
									<button class="delete-file display-none" type="button"></button>
								</label>
								<?/*
								<label class="order-order__file" for="order-order__file">Загрузить файл</label>
								<input class="order__input" id="order-order__file" type="file" name="<?= $name ?>" placeholder="Прикрепить файл" onchange="processSelectedFiles(this, '.order-order', '.order-order__file')">
								*/?>
							<? } else { ?>
