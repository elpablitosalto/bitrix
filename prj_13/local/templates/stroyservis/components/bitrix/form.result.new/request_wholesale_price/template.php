...
?>
<div class="popup order-wholesale js_popup_order_wholesale" id="js_popup_order_wholesale">
	...
	<? } ?>
	<div class="js_request_wholesale_price_form_container">
		<?
		...
		<? } ?>
		<? if ($arResult["isFormNote"] != "Y") { ?>
			<h2 class="js_h2_wholesale_know js_h2_wholesale display-none">Запросить оптовую цену</h2>
			<h2 class="js_h2_wholesale_know js_h2_know display-none">Узнать цену</h2>
			<p class="popup-form__text">Оставьте свои контактные данные и наши специалисты по проектной работе свяжутся с вами.</p>
			<div class="order-wholesale__form">
				<?
				foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
					if (isset($arParams[$FIELD_SID]) /*&& $arParams[$FIELD_SID]['VALUE']*/ && $arParams[$FIELD_SID]['AUTOCOMPLETE'] == 'Y') {
						$arQuestion['HTML_CODE'] = str_replace('name=', 'class="js_product_name_form_request_wholesale_price" value="' . $arParams[$FIELD_SID]['VALUE'] . '" name=', $arQuestion['HTML_CODE']);
					}
					//vardump($arQuestion);
