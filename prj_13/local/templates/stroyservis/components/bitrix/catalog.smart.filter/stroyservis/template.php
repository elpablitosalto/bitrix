...
<?*/ ?>
<input type="hidden" id="filter_form_id" value="<? echo $arResult["FILTER_NAME"] . "_form" ?>" />
...
<div class="page-main__filter">
	...
	//vardump($_REQUEST);
	?>
	<form id="<? echo $arResult["FILTER_NAME"] . "_form" ?>" name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>" method="get" class="smartfilter">
		<?
		$bShowSortField = true;
		$bShowSortOrder = true;
		foreach ($arResult["HIDDEN"] as $arItem) {
			if ($arItem["CONTROL_NAME"] == 'ELEMENT_SORT_FIELD') {
				$bShowSortField = false;
			}
			if ($arItem["CONTROL_NAME"] == 'ELEMENT_SORT_ORDER') {
				$bShowSortOrder = false;
			} ?>
		...
		if ($bShowSortField) {
		?>
			<input type="hidden" value="" id="ELEMENT_SORT_FIELD" name="ELEMENT_SORT_FIELD" />
		<?
		}
		if ($bShowSortOrder) {
		?>
			<input type="hidden" value="" id="ELEMENT_SORT_ORDER" name="ELEMENT_SORT_ORDER" />
		<?
		}
		?>
		<div class="filter">
			<div class="filter__title">
				<h3>Фильтры</h3>
				<button class="filter__cancel">Закрыть</button>
			</div>
			<div class="filter__selected filter__hidden">
				<div class="filter__selected_wrapper"></div>
				<div class="filter__clear">Очистить фильтр</div>
			</div>
			<div class="filter__wrapper">
				<?if ($arParams['SECTION_ID'] > 0):?>
					<div class="category">
						<h2>Категория</h2>
						<a target="_self" class="category__item category__first" href="<?=SITE_DIR?>catalog/">Все категории</a>
						<?
						$nav = CIBlockSection::GetNavChain($arParams['IBLOCK_ID'], $arParams['SECTION_ID'], ['ID', 'NAME', 'SECTION_PAGE_URL']);
						while ($arSection = $nav->GetNext()):?>
							<a target="_self" class="category__item <?if ($arSection['ID'] == $arParams['SECTION_ID']):?>category__third category__active<?else:?>category__second<?endif;?>" href="<?=$arSection['SECTION_PAGE_URL']?>"><?=$arSection['NAME']?></a>
						<?endwhile;?>
					</div>
				<?endif;?>
				<ul class="filter__list">
						...
						?>
							...
							?>
								...
								<li class="filter__item">
									<p class="filter__item_title"><?= $arItem["NAME"] ?></p>
									<div class="filter__item-wrapper">
										...
										foreach ($arItem["VALUES"] as $val => $ar) {
											$i++;
											if ($i == 4) { ?>
												<div class="filter__item-wrapper_second">
												<?
											}
												?>
												<input class="filter__input visually-hidden" type="checkbox" name="<? echo $ar["CONTROL_NAME"] ?>" id="<? echo $ar["CONTROL_ID"] ?>" value="<? echo $ar["HTML_VALUE"] ?>" <? echo $ar["CHECKED"] ? 'checked="checked"' : '' ?> onclick="smartFilter.click(this)" />
												<label data-role="label_<?= $ar["CONTROL_ID"] ?>" class="filter__checkbox <? echo $ar["DISABLED"] ? 'disabled' : '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
													<? if ($arItem['USER_TYPE'] == 'directory' && ($arItem['CODE'] == 'COLOR_REF' || $arItem['CODE'] == 'TSVET')) { ?>
														<span style="background-color: #<?= $ar['PROPS']['UF_COLOR'] ?>;"></span>
													<? } ?>
													<?= $ar["VALUE_SHOW"]; ?>
												</label>
											<?
										...
										if ($i >= 4) {
											?>
												...
												<p class="filter__more-wrapper"><span class="filter__more">Ещё</span></p>
											<?
										}
											?>
									</div>
										...
										<div class="checkbox">
											<label data-role="label_<?= $ar["CONTROL_ID"] ?>" class="bx-filter-param-label <? echo $ar["DISABLED"] ? 'disabled' : '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
												<span class="bx-filter-input-checkbox">
													...
													<span class="bx-filter-param-text" title="<?= $ar["VALUE"]; ?>"><?= $ar["VALUE"]; ?><?
																																		if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])) :
																																		?>&nbsp;(<span data-role="count_<?= $ar["CONTROL_ID"] ?>"><? echo $ar["ELEMENT_COUNT"]; ?></span>)<? endif; ?></span>
												</span>
											</label>
										</div>
									<? endforeach; ?>
					...
					?>
					<li class="filter__item filter__controls">
						<input class="product__btn" type="submit" id="set_filter" name="set_filter" value="<?= GetMessage("CT_BCSF_SET_FILTER") ?>" />
					</li>
				</ul>
			</div>
		</div>
		...
		<input type="hidden" id="js_run_slider" value="N" />
	</form>
	<?
	//vardump($arResult["JS_FILTER_PARAMS"]);
	?>
</div>
