...
$isActiveBlock = in_array('delivery', $arOrderBlocks);
$deliveryDescription = '';
?>
...
<div class="order__section">
	<h2 class="order__section-title<?if ($isActiveBlock):?> order__section_open<?endif;?>">Доставка</h2>
	<input class="order__block-toggle" type="checkbox" name="order_blocks[]" value="delivery"<?if ($isActiveBlock):?> checked<?endif;?>>
	<div class="order__body"<?if (!$isActiveBlock):?> style="display: none;"<?endif;?>>
		<?
		if(!empty($arResult["DELIVERY"]))
		{
			?>
			<ul class="order__delivery">
				<?
					...
					if ($delivery_id !== 0 && intval($delivery_id) <= 0)
					{
						foreach ($arDelivery["PROFILES"] as $profile_id => $arProfile)
						{
							...
							<li>
								...
								<label for="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>" class="order__type-entity">
									...
									if ($arProfile["CHECKED"] == "Y") {
										if (strlen($arProfile["DESCRIPTION"]) > 0) {
											$deliveryDescription = nl2br($arProfile["DESCRIPTION"]);
										} else {
											$deliveryDescription = nl2br($arDelivery["DESCRIPTION"]);
										}
									}
									?>
					...
					else // stores and courier
					{
						...
						<li>
							<input type="radio"
								   ...
								   class="visually-hidden"
							/>
							<label for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>" class="order__type-entity" <?=$clickHandler?>>
							...
							if ($arDelivery["CHECKED"] == "Y" && strlen($arDelivery["DESCRIPTION"]) > 0)
								$deliveryDescription = $arDelivery["DESCRIPTION"];
							?>
						</li>
						<?
					}
		...
		?>
		<?if (mb_strlen($deliveryDescription) > 0):?>
