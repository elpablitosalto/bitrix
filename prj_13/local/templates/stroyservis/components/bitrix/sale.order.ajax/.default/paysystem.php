...
$isActiveBlock = in_array('paysystem', $arOrderBlocks);
$paymentDescription = '';
?>

<div class="order__section">
	<h2 class="order__section-title<?if ($isActiveBlock):?> order__section_open<?endif;?>">Способ оплаты</h2>
	<input class="order__block-toggle" type="checkbox" name="order_blocks[]" value="paysystem"<?if ($isActiveBlock):?> checked<?endif;?>>
					...
					if (count($arResult["PAY_SYSTEM"]) == 1)
					{
						...
						<li>
							...
							<label for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
								<?if (strlen($arPaySystem['CODE']) > 0):?>
									<svg width="23" height="19">
										<use xlink:href="<?=SITE_TEMPLATE_PATH?>/img/icons/sprite/svg-sprite.svg#<?=$arPaySystem['CODE']?>"></use>
									</svg>
								<?endif;?>
								...
								if ($arPaySystem["CHECKED"] == "Y")
									$paymentDescription = $arPaySystem["DESCRIPTION"];
								?>
								<?/*<p>
									<?
