...
if (!function_exists("PrintSinglePropForm"))
{
	function PrintSinglePropForm($arProperties, $locationTemplate = ".default")
	{
		if (!empty($arProperties))
		{
			...
			?>
			<div class="form__input order__input-<?=tolower($arProperties["CODE"])?>">
				<?
				if ($arProperties["TYPE"] == "CHECKBOX")
				{
					?>
				...
				elseif ($arProperties["TYPE"] == "TEXT")
				{
					$isRecipientField = (strpos($arProperties['CODE'], 'RECIPIENT') !== false);
					?>
					<label class="visually-hidden" for="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties["NAME"]?><?if ($arProperties["REQUIED_FORMATED"]=="Y" || $isRecipientField):?>*<?endif;?></label>
					<input
						type="<?if (in_array($arProperties['CODE'], ['INN', 'KPP'])):?>number<?else:?>text<?endif;?>"
						maxlength="250"
						size="<?=$arProperties["SIZE1"]?>"
						value="<?=$arProperties["VALUE"]?>"
						name="<?=$arProperties["FIELD_NAME"]?>"
						id="<?=$arProperties["FIELD_NAME"]?>"
						class="order__input<?if ($arProperties['VALIDATE']['HAS_ERROR'] == 'Y'):?> form__error<?endif;?><?if (strpos($arProperties['CODE'], 'PHONE') !== false):?> js_phone_class<?endif;?>"
						placeholder="<?=$arProperties["NAME"]?><?if ($arProperties["REQUIED_FORMATED"]=="Y" || $isRecipientField):?>*<?endif;?>"
						<?/*if ($arProperties["REQUIED_FORMATED"]=="Y"):?>required<?endif;*/?>
						<?/*if (strpos($arProperties['CODE'], 'EMAIL') !== false):?>pattern="email"<?endif;*/?>
						data-field-code="<?=$arProperties["CODE"]?>"
					/>
					<?if ($arProperties['VALIDATE']['HAS_ERROR'] == 'Y'):?>
						<label id="<?=$arProperties["FIELD_NAME"]?>-error" class="form__error" for="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties['VALIDATE']['MESSAGE']?></label>
					<?endif;?>
					<?
				}
				elseif ($arProperties["TYPE"] == "SELECT")
				...
				elseif ($arProperties["TYPE"] == "TEXTAREA")
				{
					...
					?>
					<label class="visually-hidden" for="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties["NAME"]?><?if ($arProperties["REQUIED_FORMATED"]=="Y" || $isRecipientField):?>*<?endif;?></label>
					<textarea class="order__textarea<?if ($arProperties['VALIDATE']['HAS_ERROR'] == 'Y'):?> form__error<?endif;?>" placeholder="<?=$arProperties["NAME"]?><?if ($arProperties["REQUIED_FORMATED"]=="Y" || $isRecipientField):?>*<?endif;?>" rows="<?=$rows?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties["VALUE"]?></textarea>
					<?if ($arProperties['VALIDATE']['HAS_ERROR'] == 'Y'):?>
						<label id="<?=$arProperties["FIELD_NAME"]?>-error" class="form__error" for="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties['VALIDATE']['MESSAGE']?></label>
					<?endif;?>
					<?
