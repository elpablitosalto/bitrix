...
?>
    ...
    <ul class="contacts__list">
        <? foreach ($arResult["ITEMS"] as $key => $arItem) { ?>
            <li class="contacts__item"><a href="#<?= $this->GetEditAreaId($arItem['ID']); ?>"><?= $arItem['NAME']; ?></a></li>
        <? } ?>
        <li class="contacts__item"><a href="#requisites">Реквизиты</a></li>
    </ul>
...
<? if (!empty($arResult["ITEMS"])) { ?>
    ...
    <? foreach ($arResult["ITEMS"] as $key => $arItem) { ?>
        ...
        ?>
        <div class="contacts__address" id="<?= $this->GetEditAreaId($arItem['ID']); ?>" itemscope itemtype="https://schema.org/Organization">
            <div class="contacts__maps">
                <h2 itemprop="name"><?= $arItem['NAME']; ?></h2>
                <div class="contacts__map-wrapper">
                    <div class="contacts__map-address" itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
                    ...
                    <div class="contacts__map-constact">
                        <? foreach ($arItem['PHONES']['arSources'] as $k => $phone) { ?>
                            <a class="contacts__map-phone" itemprop="telephone" href="tel:<?= $arItem['PHONES']['arValuesForLink'][$k] ?>"><span itemprop="telephone"><?= $phone; ?></span></a>
                        <? } ?>
                        <? if (!empty($arItem['DISPLAY_PROPERTIES']['EMAIL']['VALUE'])) { ?>
                            <a itemprop="email" href="mailto:<?= $arItem['DISPLAY_PROPERTIES']['EMAIL']['VALUE'] ?>"><?= $arItem['DISPLAY_PROPERTIES']['EMAIL']['VALUE'] ?></a>
                        <? } ?>
                    </div>
                </div>
                <div class="contacts__manager">
                    <? if (!empty($arItem['MANAGER_PHOTO'])) { ?>
                        <div class="contacts__manager-image">
                            <img src="<?= $arItem['MANAGER_PHOTO']['SRC'] ?>" alt="<?= $arItem['MANAGER_PHOTO']['ALT'] ?>" title="<?= $arItem['MANAGER_PHOTO']['TITLE'] ?>" />
                        </div>
                    <? } ?>
                    <div class="contacts__manager-name">
                        ...
                        <? if (!empty($arItem['WHATSAPP_PHONE_FOR_LINK'])) { ?>
                            <a class="contacts__button-messenger" target="_blank" href="https://wa.me/<?= $arItem['WHATSAPP_PHONE_FOR_LINK']; ?>">Cвязаться в Whatsapp
                ...
                <div class="contacts__maps-wrapper">
                    <? if ($arItem['DISPLAY_PROPERTIES']['NEED_ORDER_PASS']['VALUE_XML_ID'] == 'Y' && !empty($arItem['PASS_PHONE_FOR_LINK'])) { ?>
                        <div class="contacts__map-pass">
                            Не забудьте заказать пропуск
                            <a class="contacts__button-pass" target="_blank" href="https://wa.me/<?= $arItem['PASS_PHONE_FOR_LINK']; ?>">Заказать пропуск</a>
                        </div>
                    <? } ?>
                    <? if (!empty($arItem['DISPLAY_PROPERTIES']['ROAD_MAP']['VALUE'])) { ?>
                        <a class="contacts__button-map" target="_blank" href="<?= $arItem['DISPLAY_PROPERTIES']['ROAD_MAP']['VALUE']; ?>" target="_blank">Карта проезда</a>
                    <? } ?>
                    <? if (!empty($arItem['DISPLAY_PROPERTIES']['LONGITUDE']['VALUE']) && !empty($arItem['DISPLAY_PROPERTIES']['LATITUDE']['VALUE'])) { ?>
                        <a class="contacts__button-route" target="_blank" href="https://yandex.ru/maps/?rtext=~<?= $arItem['DISPLAY_PROPERTIES']['LONGITUDE']['VALUE']; ?>,<?= $arItem['DISPLAY_PROPERTIES']['LATITUDE']['VALUE'] ?>&rtt=auto">
                            <svg width="24" height="24">
                                <use xlink:href="<?= SITE_TEMPLATE_PATH ?>/img/icons/sprite/svg-sprite.svg#route"></use>
                            </svg> Построить машрут
                        </a>
                    <? } ?>
                </div>
                <div class="map-wrapper js_maps_contacts" id="<?= md5($arItem['ID']); ?>" data-marker-img="<?= SITE_TEMPLATE_PATH ?>/img/design/marker-maps.svg" data-hint="<?= $arItem['NAME']; ?>" data-longitude="<?= $arItem['DISPLAY_PROPERTIES']['LONGITUDE']['VALUE']; ?>" data-latitude="<?= $arItem['DISPLAY_PROPERTIES']['LATITUDE']['VALUE'] ?>"></div>
            </div>
