...
if (isset($arResult['ITEM'])) {
	...
	foreach ($morePhoto as $key => $photo) {
		...
		/*
		if (!is_file($_SERVER["DOCUMENT_ROOT"] . $photo['SRC'])) {

			$file_path = $this->GetFolder() . '/images/no_photo.png';

			//echo 'file_path = ' . $file_path . '<br />';

			$morePhoto[$key]['SRC'] = $file_path;
		}
		*/
	}
...
?>
	...
	<li class="search__item" id="<?= $areaId ?>" data-entity="item">
		<?
		...
		if (isset($jsParams['BASKET']['ADD_URL_TEMPLATE']))
			$jsParams['BASKET']['ADD_URL_TEMPLATE'] = '/?' . explode('?', $jsParams['BASKET']['ADD_URL_TEMPLATE'])[1];

		if (isset($jsParams['BASKET']['BUY_URL_TEMPLATE']))
			$jsParams['BASKET']['BUY_URL_TEMPLATE'] = '/?' . explode('?', $jsParams['BASKET']['BUY_URL_TEMPLATE'])[1];
		?>
		<script>
