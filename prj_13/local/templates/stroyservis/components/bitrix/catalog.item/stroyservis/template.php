...
if (isset($arResult['ITEM'])) {
	...
	$morePhoto = $morePhotoTmp;

	$item['PICTURES'] = array();
	$item['PICTURES'][] = $item['PICTURE'];
	foreach ($morePhoto as $key => $arPict) {
		$item['PICTURES'][] = $arPict;
	}

	// Уберем дубликаты изображений -->
	if (!empty($item['PICTURES'])) {
		$arPicts_1 = $item['PICTURES'];
		$arPicts_2 = $item['PICTURES'];
		$arPictsTmp = array();
		foreach ($arPicts_1 as $key => $arPict) {
			$bAdd = true;
			foreach ($arPictsTmp as $key_2 => $arPict_2) {
				if (
					$arPict['SOURCE_PICTURE']['HEIGHT'] == $arPict_2['SOURCE_PICTURE']['HEIGHT']
					&& $arPict['SOURCE_PICTURE']['WIDTH'] == $arPict_2['SOURCE_PICTURE']['WIDTH']
					&& $arPict['SOURCE_PICTURE']['FILE_SIZE'] == $arPict_2['SOURCE_PICTURE']['FILE_SIZE']
					&& $arPict['SOURCE_PICTURE']['CONTENT_TYPE'] == $arPict_2['SOURCE_PICTURE']['CONTENT_TYPE']
				) {
					$bAdd = false;
				}
			}
			if ($bAdd) {
				$arPictsTmp[$key] = $arPict;
			}
		}
		$item['PICTURES'] = $arPictsTmp;
	}
	// <--
	...
	*/
	...
	$bestseller_class = '';
	if ($item['PROPERTIES']['BESTSELLER']['VALUE_XML_ID'] == 'Y') {
		//$bestseller_class = 'product__item_best-seller';
	}
?>
	<li class="product__item <?= $bestseller_class; ?>" itemtype="https://schema.org/Product" itemprop="itemListElement" itemscope="itemscope" id="<?= $areaId ?>" data-entity="items-row">
	<?/*<div class="product-item-container<?= (isset($arResult['SCALABLE']) && $arResult['SCALABLE'] === 'Y' ? ' product-item-scalable-card' : '') ?>" id="<?= $areaId ?>" data-entity="item">*/?>
		<?
		...
		//vardump($item['PRODUCT_PROPERTIES']);
		//vardump($arParams);
