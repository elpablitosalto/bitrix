...
<script id="basket-item-template" type="text/html">
	<li class="basket__item basket-items-list-item-container{{#SHOW_RESTORE}} basket-items-list-item-container-expend{{/SHOW_RESTORE}}"
		id="basket-item-{{ID}}" data-entity="basket-item" data-id="{{ID}}">
		<div class="basket__item-checkbox">
			<input class="visually-hidden js-choose-basket-item" id="choose-basket-item-{{ID}}" type="checkbox">
			<label for="choose-basket-item-{{ID}}"></label>
		</div>
		{{#SHOW_RESTORE}}
		...
		<div class="basket__item-wrapper">
			<div class="basket__item-main">
				{{#DETAIL_PAGE_URL}}
				<a href="{{DETAIL_PAGE_URL}}" class="basket__title">
					{{/DETAIL_PAGE_URL}}
					...
					foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName)
					{
						switch (trim((string)$blockName))
						{
							...
							case 'columns':
								...
								{{#IS_TEXT}}
								<div class="basket__code basket-item-property-custom basket-item-property-custom-text
												{{#HIDE_MOBILE}}hidden-xs{{/HIDE_MOBILE}}"
									 data-entity="basket-item-property">
				...
				if ($useSumColumn)
				{
					...
					<div class="basket__item-price">
						<div class="basket__item-sum" id="basket-item-sum-price-{{ID}}">{{{SUM_PRICE_FORMATED}}}</span>
						</div>
						<div class="basket__item-sumfor">{{{PRICE_ONE_KG_FORMATED}}}</span></div>
					</div>
					{{#SHOW_DISCOUNT_PRICE}}
						<div class="basket__item-oldprice">
							<div class="basket__item-old" id="basket-item-sum-price-old-{{ID}}">{{{SUM_FULL_PRICE_FORMATED}}}</span>
							</div>
							{{#DISCOUNT_PRICE_PERCENT}}
								<div class="basket__item-promotion">
			...
			<div class="basket__item-add" data-entity="basket-item-quantity-block">
				<button class="basket__add_control basket__add_control_minus" data-entity="basket-item-quantity-minus">-</button>
				<input size="2" type="text" class="basket__input-quantity" value="{{QUANTITY}}"
					   {{#NOT_AVAILABLE}} disabled="disabled"{{/NOT_AVAILABLE}}
				data-value="{{QUANTITY}}" data-entity="basket-item-quantity-field"
				id="basket-item-quantity-{{ID}}">
				<button class="basket__add_control basket__add_control_plus" data-entity="basket-item-quantity-plus">+</button>
			</div>
		</div>
		<?
		...
		if (isset($mobileColumns['DELETE']))
		{
			...
			<div class="basket__item-delete">
				<button class="basket__item-button" data-entity="basket-item-delete">
					<svg width="18" height="20">
						<use xlink:href="<?=SITE_TEMPLATE_PATH?>/img/icons/sprite/svg-sprite.svg#delete"></use>
					</svg>
				</button>
			</div>
			<?
