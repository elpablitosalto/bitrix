...
foreach ($this->basketItems as $row)
{
	$rowData = array(
		...
		'SUM_PRICE_FORMATED' => str_replace('&#8381;', '<span class="basket__price_ruble">&#8381;</span>', $row['SUM']),
	...
	if (!empty($result['GRID']['HEADERS']) && is_array($result['GRID']['HEADERS']))
	{
		...
		foreach ($result['GRID']['HEADERS'] as &$value)
		{
			...
			elseif (!empty($row[$value['id']]))
			{
                ...
                if ($value['id'] == 'PROPERTY_VES_ATTR_S_VALUE' && $rawValue > 0) {
                    $rowData['PRICE_ONE_KG'] = $rowData['PRICE'] / $rawValue;
                    $rowData['PRICE_ONE_KG_FORMATED'] = CurrencyFormat($rowData['PRICE_ONE_KG'], 'RUB') . '/кг.';
                    $totalWeight += ($rawValue * $rowData['QUANTITY']);
                } else {
                    $rowData['COLUMN_LIST'][] = array(
                        'CODE' => $value['id'],
                        'CODE333' => $value['id'],
...
$totalData = array(
	...
	'BONUS' => Indexis::getBonusValueByPrice($result['allSum']),
...
//}
$totalData['WEIGHT_FORMATED'] = $totalWeight . ' кг';
$totalData['PRODUCT_COUNT'] = count($result['BASKET_ITEM_RENDER_DATA']);
$totalData['PRODUCT_COUNT_FORMATED'] = Indexis::num2word($totalData['PRODUCT_COUNT'], ['#NUM# товар', '#NUM# товара', '#NUM# товаров']);
