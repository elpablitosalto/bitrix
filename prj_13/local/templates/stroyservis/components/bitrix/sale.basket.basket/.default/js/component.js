		...
		fillTotalBlocks: function()
		{
			...
			if (totalNodes && totalNodes.length)
			{
				...
				if (totalTemplate)
				{
					...
					if (this.result.TOTAL_RENDER_DATA.PRODUCT_COUNT == 0)
						location.reload();
			...
			document.querySelector('[data-entity="total-weight-value"]').innerHTML = this.result.TOTAL_RENDER_DATA.WEIGHT_FORMATED;
			document.querySelector('[data-entity="total-product-count-value"]').innerHTML = this.result.TOTAL_RENDER_DATA.PRODUCT_COUNT_FORMATED;
			document.querySelector('[data-entity="mobile-total-product-count-value"]').innerHTML = this.result.TOTAL_RENDER_DATA.PRODUCT_COUNT;
			document.querySelector('[data-entity="mobile-total-price"]').innerHTML = this.result.TOTAL_RENDER_DATA.PRICE_FORMATED.replace('&#8381;', '<span class="basket__mobile-ruble">&#8381;</span>');
			...
			if (startPrice !== finalPrice)
			{
				...
				new BX.easing({
					...
					step: BX.delegate(function(state){
						...
						priceNode.innerHTML = this.getFormatPrice(state.price, itemData.CURRENCY).replace('&#8381;', '<span class="basket__price_ruble">&#8381;</span>');
					}, this),
					complete: BX.delegate(function() {
						var node, price;

						priceNode.innerHTML = this.getFormatPrice(finalPrice, itemData.CURRENCY).replace('&#8381;', '<span class="basket__price_ruble">&#8381;</span>');
