...
<div id="js_reg_modal_container">
	...
	<? } else { ?>
		...
		if (!empty($arResult["ERRORS"])) {
			...
			$str = implode("<br />", $arResult["ERRORS"]);
			$str = html_entity_decode($str);
			echo $str;
			//ShowError($str);
		}
		?>
		<form class="registration-form js_validate_ajax" data-type-form="PARTNER_REG" data-scroll-to="js_reg_modal_container" data-container-id="js_reg_modal_container" method="post" action="<?= POST_FORM_ACTION_URI ?>">
			<input type="hidden" name="REG_FORM" value="Y" />
			<input type="hidden" name="register_submit_button" value="Y" />
			<input type="hidden" name="REGISTER[LOGIN]" value="YYYY" />
			<input type="hidden" name="REGISTER[UF_PARTNER]" value="Y" />
			<input type="hidden" name="REGISTER[PASSWORD]" value="<?=$arResult['PASSWORD_GEN'];?>" />
			<input type="hidden" name="REGISTER[CONFIRM_PASSWORD]" value="<?=$arResult['PASSWORD_GEN'];?>" />

			<div class="registration-form__wrapper">
				<h2>Стать партнером</h2><a href="#" class="js_auth_popup">Я уже партнер</a>
			</div>
			<?
			...
			<div class="registration-form__policy">Нажимая на кнопку «Отправить заявку», вы соглашаетесь с <a href="<?= $GLOBALS["arSiteConfig"]["LINKS"]['PERSONAL_DATA_AGREEMENT'] ?>">политикой обработки персональных данных</a>
			</div>
			<button class="link-button_rose js_link_button_registration" type="submit">Отправить заявку</button>
		</form>
	<? } ?>
</div>