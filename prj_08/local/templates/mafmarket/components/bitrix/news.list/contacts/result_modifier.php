...
foreach ($arResult["ITEMS"] as &$arItem) {
    ...
    // Как добраться, фото -->
    if (!empty($arItem['DISPLAY_PROPERTIES']['HOW_TO_GET_PICT_1']['FILE_VALUE'])) {
        $arResultLocal = Indexis::getImageFormatted(array(
            'RESIZE' => 'N',
            'FILE_VALUE' => $arItem['DISPLAY_PROPERTIES']['HOW_TO_GET_PICT_1']['FILE_VALUE'],
            'NO_IMAGE_DEFAULT' => $this->GetFolder() . '/images/reviewer-thumb.png',
            //'WIDTH' => 205,
            //'HEIGHT' => 116,
            'DEFAULT_ALT_TITLE' => $arItem['NAME']
        ));
        $arItem['HOW_TO_GET_PICT_1'] = $arResultLocal['PICTURE'];
    }

    if (!empty($arItem['DISPLAY_PROPERTIES']['HOW_TO_GET_PICT_2']['FILE_VALUE'])) {
        $arResultLocal = Indexis::getImageFormatted(array(
            'RESIZE' => 'N',
            'FILE_VALUE' => $arItem['DISPLAY_PROPERTIES']['HOW_TO_GET_PICT_2']['FILE_VALUE'],
            'NO_IMAGE_DEFAULT' => $this->GetFolder() . '/images/reviewer-thumb.png',
            //'WIDTH' => 205,
            //'HEIGHT' => 116,
            'DEFAULT_ALT_TITLE' => $arItem['NAME']
        ));
        $arItem['HOW_TO_GET_PICT_2'] = $arResultLocal['PICTURE'];
    }
    // <-- Как добраться, фото
