...
while($ob = $res->GetNext()){
    ...
    $arResult["REFERENCE"][$iblocksReference[$ob["IBLOCK_ID"]]][$ob["NAME"]] = $ob["PREVIEW_PICTURE_SRC"];
}

if(!empty($arItemsID)){
    $noPhoto = "/local/templates/mafmarket/components/bitrix/sale.basket.basket/main/images/no_photo.png";
    $arItemProps = [];
    $arSelect = Array(
        "ID",
    ...
    $propsByReference = [
        "PROPERTY_VID_DREVESINY_BRUSKA_DOSKI" => "tree_types",
        "PROPERTY_METALL" => "metal_types",
        "PROPERTY_OKRAS_BRUSKA" => "color_options",
        "PROPERTY_TSVET_METALLICHESKOGO_POKRYTIYA" => "colors",
    ];
    $arFilter = Array("IBLOCK_ID"=>Indexis::getIblockId("catalog_1c_offers", "1c_catalog"), "ID"=>$arItemsID);
    $res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nTopCount"=>count($arItemsID)), $arSelect);
    while($ob = $res->GetNext())
    {
        foreach($propsByReference as $propCode => $referenceCode){
            if(mb_strlen($ob[$propCode."_VALUE"]) > 0){
                $ob[$propCode."_VALUE"] = mb_strtolower($ob[$propCode."_VALUE"]);
                if(isset($arResult["REFERENCE"][$referenceCode][ $ob[$propCode."_VALUE"]])){
                    $ob[$propCode."_IMAGE"] = $arResult["REFERENCE"][$referenceCode][ $ob[$propCode."_VALUE"]];
                } else {
                    $ob[$propCode."_IMAGE"] = $noPhoto;
                }
            }
        }

        $arItemProps[$ob["ID"]] = $ob;
    }
}
...
foreach ($this->basketItems as $row)
{
	$rowData = array(
		'ID' => $row['ID'],
		//'ARTICLE' => $arArticles[$row['PRODUCT_ID']],
        'VID_DREVESINY_BRUSKA_DOSKI' => $arItemProps[$row['PRODUCT_ID']]["PROPERTY_VID_DREVESINY_BRUSKA_DOSKI_VALUE"],
        'VID_DREVESINY_BRUSKA_DOSKI_IMAGE' => $arItemProps[$row['PRODUCT_ID']]["PROPERTY_VID_DREVESINY_BRUSKA_DOSKI_IMAGE"],
        'METALL' => $arItemProps[$row['PRODUCT_ID']]["PROPERTY_METALL_VALUE"],
        'METALL_IMAGE' => $arItemProps[$row['PRODUCT_ID']]["PROPERTY_METALL_IMAGE"],
        'OKRAS_BRUSKA' => $arItemProps[$row['PRODUCT_ID']]["PROPERTY_OKRAS_BRUSKA_VALUE"],
        'OKRAS_BRUSKA_IMAGE' => $arItemProps[$row['PRODUCT_ID']]["PROPERTY_OKRAS_BRUSKA_IMAGE"],
        'TSVET_METALLICHESKOGO_POKRYTIYA' => $arItemProps[$row['PRODUCT_ID']]["PROPERTY_TSVET_METALLICHESKOGO_POKRYTIYA_VALUE"],
        'TSVET_METALLICHESKOGO_POKRYTIYA_IMAGE' => $arItemProps[$row['PRODUCT_ID']]["PROPERTY_TSVET_METALLICHESKOGO_POKRYTIYA_IMAGE"],
    ...
    if($rowData["OBEM"]){
        $rowData["VOLUME_VAL"] = $rowData["OBEM"];
        $fullVolume += $rowData["QUANTITY"]*(int)preg_replace('/\D/', '', $rowData["OBEM"]);
    }
...
if($fullVolume > 0){
    $totalData['VOLUME_FORMATED'] = $fullVolume." л.";
}
