...
<div class="container">
    ...
    <? if ($arResult['SHOW_CLOSE_NEWS'] == 'N') { ?>
        <div class="row">
            <div class="col-lg-12">
                <?
                ShowError('Доступ к новости закрыт.');
                ?>
            </div>
        </div>
    <? } else { ?>

        <? if (!empty($arResult["arPictSizes"]["BIG"]["src"])) { ?>
            <div class="ml-banner ml-news-detail__cover">
                <picture class="ml-banner__img">
                    <? if (!empty($arResult["arPictSizes"]["SMALL"]["src"])) { ?>
                        <source media="(max-width: 480px)" data-srcset="<?= $arResult["arPictSizes"]["SMALL"]["src"] ?>" srcset="<?= $arResult["arPictSizes"]["MEDIUM"]["src"] ?>">
                    <? } ?>
                    <? if (!empty($arResult["arPictSizes"]["MEDIUM"]["src"])) { ?>
                        <source media="(max-width: 991px)" data-srcset="<?= $arResult["arPictSizes"]["MEDIUM"]["src"] ?>" srcset="<?= $arResult["arPictSizes"]["MEDIUM"]["src"] ?>">
                    <? } ?>
                    <img class="lazyload" data-src="<?= $arResult["arPictSizes"]["BIG"]["src"] ?>" src="<?= $arResult["arPictSizes"]["BIG"]["src"] ?>" width="1720" height="660" alt="">
                </picture>
            </div>
        <? } ?>
                ...
                <div class="row">
                    <div class="col-lg-9">
                        ...
                        <?if($arResult['PROPERTIES']['LINK_BTN']['VALUE']):?>
                            <div><a class="ml-btn ml-btn_round" href="<?=$arResult['PROPERTIES']['LINK_BTN']['VALUE']?>"><?=$arResult['PROPERTIES']['TEXT_BTN']['VALUE'];?></a></div>
                        <?endif;?>
                    </div>
                </div>
            </div>
