...
<?if (count($arResult['ITEMS']) > 0):?>
    <?foreach($arResult['ITEMS'] as $arItem):?>
        ...
        <a href="<?=$arItem['DISPLAY_PROPERTIES']['LINK']['VALUE']?>">
            <picture class="home-top-banner" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
                <?if (isset($arItem['DISPLAY_PROPERTIES']['BANNER_SMARTPHONE']['FILE_VALUE']['SRC'])):?>
                    <source media="(max-width: 575px)" srcset="<?=$arItem['DISPLAY_PROPERTIES']['BANNER_SMARTPHONE']['FILE_VALUE']['SRC']?>" />
                <?endif;?>
