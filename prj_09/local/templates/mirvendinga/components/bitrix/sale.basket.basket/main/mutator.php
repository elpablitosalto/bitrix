...
{
    $packageAmount = \Mirvendinga\Catalog::getProductPackageAmount($row["PRODUCT_ID"]);
	$rowData = array(
		'ID' => $row['ID'],
		...
		'MEASURE_TEXT' => $packageAmount['MEASURE'],
		'AVAILABLE_QUANTITY' => $row['AVAILABLE_QUANTITY'],
		'CHECK_MAX_QUANTITY' => $row['CHECK_MAX_QUANTITY'],
		'SHOW_AVAILABLE_QUANTITY_BOOL' => $row['AVAILABLE_QUANTITY'] < 999,
		'MODULE' => $row['MODULE'],
	...
	foreach($this->arParams['SHORT_PROPERTIES'] as $line) {
		$arLine = explode('|', $line);
		...
		if(!empty($propKey) && !empty($arProp)) {
			$value = !empty($arProp['VALUE']) ? $arProp['VALUE'] : '';
		}

		if(!empty($value)) {
			$rowData['SHORT_PROPERTY_'.$propKey] = $value;
		}
	}
