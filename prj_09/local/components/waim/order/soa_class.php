...
/**
 * Class SaleOrderAjax
 * @see /bitrix/components/bitrix/sale.order.ajax/class.php
 */
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function initProperties(Order $order, $isPersonTypeChanged)
	{
		...
		$shouldUseProfile = ($firstLoad || $justAuthorized || $isProfileChanged) && !$isPersonTypeChanged;
		...
		foreach ($this->getFullPropertyList($order) as $property)
		{
			...
			if ($property['TYPE'] === 'LOCATION' && empty($curVal))
			{
				...
				$locCode = $this->getCurrentCityId(); // ID местоположения
	...
	protected function initUserProfiles(Order $order, $isPersonTypeChanged)
	{
		...
		{
			if (!$bFirst && (($profileIsNotSelected || $justAuthorized) && !$isPersonTypeChanged))
			{
				$bFirst = true;
	...
	protected function saveOrderAjaxAction()
	{
		...
		if ($this->checkSession)
		{
            ...
            foreach ($basket->getBasketItems() as $basketItem){
                $arCatalogData = \Mirvendinga\Catalog::getProductCatalogData($basketItem->getProductId());
                if(
                    !empty($arCatalogData["UNIT_PRICE"]["VALUE"]) &&
                    $arCatalogData["PACKAGE_AMOUNT"]["IS_SOLD_IN_PACKS"] &&
                    $arCatalogData["PACKAGE_AMOUNT"]["PACKAGE_AMOUNT"] > 1
                ){
                    $basketItem->setFields(array(
                        'QUANTITY' => $basketItem->getQuantity() * intval($arCatalogData["PACKAGE_AMOUNT"]["PACKAGE_AMOUNT"]),
                        'PRICE' => Sale\PriceMaths::roundPrecision($arCatalogData["UNIT_PRICE"]["VALUE"]),
                    ));
                }
            }
            $basket->save();
	...
	protected function showOrderAction()
	{
		...
		$orderId = !empty($this->request->get('ORDER_ID')) ? urldecode($this->request->get('ORDER_ID')) : $this->arResult["ORDER_ID"];
