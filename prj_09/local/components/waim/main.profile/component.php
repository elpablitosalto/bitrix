	...
	if($strError == '')
	{
		...
		foreach($arEditFields as $field)
		{
			if(isset($_REQUEST[$field]))
			{
				...
				if($field === 'FULL_NAME' && !empty($_REQUEST[$field])) {
					$arName = explode(' ', $_REQUEST[$field]);
					$arName = array_filter($arName, function($str) {
						return !empty(trim($str));
					});
					$arName = array_values($arName);

					$arFields['LAST_NAME'] = !empty($arName[0]) ? $arName[0] : '';
					$arFields['NAME'] = !empty($arName[1]) ? $arName[1] : '';
					$arFields['SECOND_NAME'] = !empty($arName[2]) ? $arName[2] : '';
				}
			}
		}
...
$arResult["arUser"]["FULL_NAME"] = sprintf('%s %s %s', $arResult["arUser"]["LAST_NAME"], $arResult["arUser"]["NAME"], $arResult["arUser"]["SECOND_NAME"]);
