...
class PersonalOrderListComponent extends CBitrixComponent
{
	...
	protected function prepareFilter()
	{
		...
		if($_REQUEST["order_search"] <> '')
		{
            $arFilter["%ACCOUNT_NUMBER"] = trim($_REQUEST["order_search"]);
		}
		...
		if ($id)
		{
			...
			foreach ($oldBasketItems as $oldBasketItem)
			{
				...
				else
				{
                    ...
                    // 41707
                    $arCatalogData = \Mirvendinga\Catalog::getProductCatalogData($oldBasketItem->getField('PRODUCT_ID'));
                    if(!empty($arCatalogData["PRICE"]["VALUE"])){
                        $resultItem = $item->setFields(array(
                            'CUSTOM_PRICE' => "Y",
                            'PRICE' => Sale\PriceMaths::roundPrecision($arCatalogData["PRICE"]["VALUE"]),
                        ));
                    }
                    if(
                        ...
                        $resultItem = $item->setFields(array(
                            'QUANTITY' => round($oldBasketItem->getQuantity() / intval($arCatalogData["PACKAGE_AMOUNT"]["PACKAGE_AMOUNT"])),
                        ));
                    }
                    //
	...
	protected function formatResult()
	{
        ...
        $arResult["PROPS_MAP"] = $this->getOrderProperties();
		...
		if(self::isNonemptyArray($this->dbResult['ORDERS']))
		{
			foreach ($this->dbResult['ORDERS'] as $k => $orderInfo)
			{
                ...
                foreach ($arResult["PROPS_MAP"][$arOrder["PERSON_TYPE_ID"]] as $propSection => $propSectionValue) {
                    foreach ($propSectionValue as $propName => $propId) {
                        $propValue = $propertyCollection->getItemByOrderPropertyId($propId);
                        $arOrder["PROPS"][$propSection][$propName] = $propValue->getValue();
                    }
                }
                $arOrder["PROPS"]["profileName"] = $propertyCollection->getProfileName()->getValue();
			}
        ...
        while($property = $obPropResult->fetch()){
            switch ($property["CODE"]){
                ...
                case "PVZ_ADDRESS":
                    ...
                    break;
                case "CONTRACT":
                    $arProperties[$property["PERSON_TYPE_ID"]]["files"]["contract"] = $property["ID"];
                    break;
                case "PAY_CHECK":
                    $arProperties[$property["PERSON_TYPE_ID"]]["files"]["payCheck"] = $property["ID"];
                    break;
                case "GARANTY_CHECK":
                    $arProperties[$property["PERSON_TYPE_ID"]]["files"]["garantyCheck"] = $property["ID"];
                    break;
            }
        }
        return $arProperties;
