...
class BitrixTools
{
    ...
    public static function picSrcset($arParams)
    {
        $picMobile = $arParams['picMobile'];
        $picTablet = $arParams['picTablet'];
        $picDesktop = $arParams['picDesktop'];
        ...
        if (!empty($picTablet['src'])) {
            $arSrcset[] = $picTablet['src'] . ' 991w';
            $arSizes[] = '(max-width: 991px) 991px';

            $arSrcset[] = $picDesktop['src'] . '';
            $arSizes[] = '(min-width: 9999px) 9999px';
        } else {
            $arSrcset[] = $picDesktop['src'] . ' 991w';
            $arSizes[] = '(min-width: 991px) 991px';
        }
        if (!empty($arSrcset)) {
...
?>
        <img <?= $id; ?> <?= $srcset; ?> <?= $sizes; ?> alt="<?= $arParams['alt']; ?>" title="<?= $arParams['title']; ?>" class="picture__image" src="<?= $arParams['src'] ?>" width="<?= $arParams['width']; ?>" height="<?= $arParams['height']; ?>" />
    <?
    }

    public static function showImageCustom($arParams)
    {
        $src = '';
        $width = '';
        $height = '';
        if (!empty($arParams['IMAGE_ID'])) {
            $arFile = CFile::GetFileArray($arParams['IMAGE_ID']);
            $src = $arFile['SRC'];
            $width = $arFile['WIDTH'];
            $height = $arFile['HEIGHT'];
        } else if (!empty($arParams['SRC'])) {
            $src = $arParams['SRC'];
        ...
        if ($arParams['LAZY_LOAD'] == 'Y') {
            $srcAttr = 'data-src="' . $src . '"';
            $class .= ' js_lazy';
        }
    ?>
        <img class="<?= $class; ?>" <?= $srcAttr ?> width="<?= $width ?>" height="<?= $height ?>" />
<?
    }
}
