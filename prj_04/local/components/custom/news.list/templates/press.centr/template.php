...
LocalRedirect('/press-center/events/');
?>
                ...
                <div class="event-item<?=(isset($arItem['DISPLAY_PROPERTIES']['STATUS'])) ? ' _active' : ''?>">
                    <a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="event-item__image">
                        <div class="event-item__image-container">
                            <img src="<?=$pic['src']?>" style="width: 100%; height: 100%;">
                            ...
                            */?>
                            ...
                            <?if($arItem["TYPE"]):?>
                                <div class="type-block"
                                    <?if(mb_strtolower($arItem["TYPE"]) == 'семинар'):?>style="background-color:#CE0000;"
                                    <?elseif(mb_strtolower($arItem["TYPE"]) == 'мастер-класс'):?>style="background-color:#3333CC;"
                                    <?endif;?>>
                                    <span>
                                        <?=$arItem["TYPE"]?>
                                    </span>
                            ...
                            <?if($arItem["COST"]):?>
                                <div class="type-block"
                                     <?if(mb_strtolower($arItem["COST"]) == 'платный'):?>style="background-color:#FAFF00; color:black !important;"
                    ...
                    <div class="event-item__description">
                        <h3><?=$arItem['NAME']?></h3>
                        <?if ($arItem['START_DATE']) { ?>
                            <div class="event-item__description-date">
                                <p><?=$arItem['START_DATE']?></p>
                                <?if ($arItem['EVENT_TIME']) { ?>
                                    <p><?=$arItem['EVENT_TIME']?></p>
                                <?} ?>
                            </div>
                        <?} ?>
                        <?/* <div class="event-item__description-date">
                            <p><?=$arItem['START_DATE']?></p>
                            <p><?if($arItem['EVENT_TIME']){echo $arItem['EVENT_TIME'];}?></p>
                        </div> <?*/?>
