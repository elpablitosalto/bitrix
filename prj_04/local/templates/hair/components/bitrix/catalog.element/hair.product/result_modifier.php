...
while ($res = $obj->GetNextElement()) {
    ...
    foreach ($arProps as $prop):
        if (
            $prop['CODE'] == 'VOLUME' || $prop['CODE'] == 'COLOR'
            || $prop['CODE'] == 'LINK' || $prop['CODE'] == 'MARKETPLACE_OZON'
            || $prop['CODE'] == 'MARKETPLACE_GOLDAPPLE' || $prop['CODE'] == 'MARKETPLACE_RIVE_GAUCHE'
        ) {
...
while ($res = $obj->GetNextElement()) {
    ...
    $arResult['SKU_PROPS']['SELECT_VALUE']['COLOR_' . $current_res['COLOR']['VALUE'] . '_VOLUME_' . $current_res['VOLUME']['VALUE']] = array(
        ...
        'LINK_RIVE_GAUCHE' => !empty($current_res['MARKETPLACE_RIVE_GAUCHE']['VALUE']) ? $current_res['MARKETPLACE_RIVE_GAUCHE']['VALUE'] : '',
    );
}
...
if (
    ...
    || !empty($arResult['PROPERTIES']['MARKETPLACE_RIVE_GAUCHE']['VALUE'])
    ...
    $arResult['MARKETPLACE_LINKS'][] = addMarketplaceLink(($arResult['PROPERTIES']['MARKETPLACE_RIVE_GAUCHE']['VALUE'] ?: '#'), $marketplaceArray["rive-gauche"]);
}